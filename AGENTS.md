# SW_MUSH — Codex Project Memory

Star Wars D6 MUSH, Clone Wars era (~20 BBY), WEG Revised & Expanded ruleset.
Stack: Python 3.14 / aiohttp / aiosqlite / SQLite, vanilla-JS SPA web client, Ollama Mistral 7B (NPC dialogue), Codex Haiku via API (Director AI, ~$20/mo circuit breaker).
...
Subagents live in `.Codex/agents/`. The main session runs on Opus — keep
...
integrity (no phantom file pointers). Add a skill = new `.Codex/skills/<name>/SKILL.md`.
