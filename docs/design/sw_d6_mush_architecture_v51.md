...
<!-- point-updated 2026-06-06: Drop 4b remainder — the dark-side cult COMMUNAL OBJECTIVE (the communal-rally villain, other half of III.3): engine/communal_objective.py (pure menace state machine) + engine/communal_objective_runtime.py (IO) + parser/communal_commands.py (`rally`) + communal_objective_tick + MIGRATIONS[43] communal_objective table. New invariant §4.31 (§4.30 reserved for the Lane A creature natural-attack contract). SCHEMA_VERSION 42 -> 43. T1.3.4 cult half DELIVERED; only the hunter's PC-death collect-consequence remains deferred. -->
<!-- NOTE: this header's "Current SCHEMA_VERSION = 40" is the 2026-06-04 snapshot; the live value is 43 (migration 41 dsp_hunter_pursuit, 42 spawned_npc_id, 43 communal_objective). -->
...
### §4.31 Communal-objective discipline (NEW v51, 2026-06-06)

*(§4.30 is reserved for the Lane A creature natural-attack faithfulness
contract.)*

The dark-side cult **communal objective** (design III.3's communal-rally
villain; the counterpart to the per-PC DSP hunter, §4.19-adjacent) is a
Director-posted, world-scale threat the whole playerbase rallies against.
Its discipline mirrors the hunter's pure/runtime/tick split and the
faucet/era/Q1 invariants:

- **Pure state machine.** `engine/communal_objective.py` owns the cult
  roster, the menace model (rises over time via `advance_menace`, pushed
  down by margin-banded strikes via `apply_strike`), win/lose resolution
  (`resolve_state`), the cross-playstyle strike pool
  (`best_strike_pool_pips` — best of combat / investigation / persuasion /
  Force so any archetype qualifies), and **every player-facing string**.
  No DB, no asyncio. `engine/communal_objective_runtime.py` owns all I/O
  (DB writes, broadcasts, director-log/news posts, reward payout); the
  cadence lives in `server/tick_handlers_progression.py::
  communal_objective_tick` (post when none active + a 6 h breather,
  escalate, resolve). Escalation **never un-wins a routed cult**:
  `advance_and_resolve` resolves an already-terminal menace before
  applying any time-based rise.

- **Opportunities, never penalties.** A community win confers status; a
  loss confers nothing and the cult merely entrenches as flavor. No
  player is debited or penalised for a communal loss.

- **Prestige-domain, faucet-disciplined (extends §4.32).** Rewards are
  Republic faction-rep (via `organizations.adjust_rep`, scaled by each
  contributor's share of total effort) plus a commemorative III.2 status
  flag (`attributes.communal_objective_wins`). **No credits are minted**;
  if a credit reward is ever added it MUST route through
  `db.adjust_credits`. Reward payout is single-writer/idempotent: state
  transitions out of `active` under an `UPDATE ... WHERE state='active'`
  rowcount guard, so a tick/strike race can never double-pay.

- **Era/Q1-clean.** All five cults are invented (no canonical figures,
  no GCW-faction strings); the roster is guarded behaviourally by
  `test_drop4b_communal_cult.py::test_roster_is_era_and_canon_clean`.

- **Deterministic + idle-Ollama flavor only (NEVER Haiku).** Like the
  hunter, optional flavor rides the idle queue via `CULT_AI_KEY`;
  deterministic fallbacks mean the objective is fully playable with the
  AI off.

**Persistence:** one `communal_objective` row per uprising (MIGRATIONS[43]);
the active one is the latest row with `state='active'`. Per-contributor
points and the per-character strike cooldown live in that row's
`contributions_json`, so reward shares need no second table.

**Deferred (flagged, not built here):** the hunter's PC-death
*collect-consequence* (the consequence when the hunter wins the climax
fight) — a separate hunter-side piece touching the combat-death hook.
The standard death penalty (respawn-Wounded + corpse + insurance) plus
the tick's escape-reconcile already cover losing to / slipping the hunter.

**Why this is an invariant:** a communal objective that minted credits
would blow the faucet discipline; one that penalised losers would violate
the opportunities-not-penalties contract; and escalation that un-won a
routed cult would make a community victory un-winnable. The pure/runtime
split keeps the whole state machine unit-testable without a live server.
...
- **v51 point-update (Jun 6 2026)** — one session, four drops (full per-drop
  detail in CHANGELOG, not duplicated here). The architecturally-significant
  delta: a **new subsystem** — the dark-side cult **communal objective** (design
  III.3's communal-rally villain, the world-scale counterpart to the per-PC DSP
  hunter): `engine/communal_objective.py` (pure menace state machine) +
  `engine/communal_objective_runtime.py` (IO) + `parser/communal_commands.py`
  (`rally`) + `communal_objective_tick` + a `@director cult` admin surface,
  recorded as **invariant §4.31** (communal-objective discipline; §4.30 reserved
  for the Lane A creature natural-attack contract), and a **schema-of-record bump
  42 → 43** (`MIGRATIONS[43]` `communal_objective`). Also this session (no
  architectural change, CHANGELOG-only): the hunter's PC-death collect-consequence
  (closing `T1.3.4`) and wiring the dormant `contraband_scan` checkpoint effect.
  Current `SCHEMA_VERSION = 43`.
