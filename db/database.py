...
class Database:
    ...
    async def get_daily_p2p_outgoing(self, char_id: int,
        ...
        """Return total credits sent by `char_id` via p2p_transfer in the
        last `seconds` (default 24 hours).

        Used to enforce a per-character daily P2P transfer cap (S51
        hardening) — friction against alt-shoveling that otherwise
        bypasses the 5% transaction tax.

        Returns 0 on table-missing or query failure (fail-open: a broken
        query should never block a legitimate transfer; the 5% tax still
        applies).

        Returns:
            int: positive integer total of credits sent (sum of
            absolute deltas of negative p2p_transfer rows for this char).
        """
        ...
        try:
            ...
            log.debug("get_daily_p2p_outgoing: query failed", exc_info=True)
            return 0
    ...
    # -- Economy alerts (S51 hardening) --
    #
    # Three signals surfaced by @economy alerts. All three are pure
    # aggregations over credit_log; the ALERT_THRESHOLDS in
    # parser/director_commands.py decide which rows are worth flagging.

    async def get_whale_transactions(
        self, threshold: int = 50000, seconds: int = 86400,
    ) -> list:
        """Return individual credit_log rows >= threshold (absolute value)
        in the last `seconds`. Used for whale alerts in @economy alerts.

        Each row carries char_id, delta (signed), source, balance,
        created_at — caller can format faucet vs sink.
        Fail-closed: returns [] on query error so an alerting bug never
        crashes the dashboard.
        """
        cutoff = time.time() - seconds
        try:
            rows = await self._db.execute_fetchall(
                "SELECT char_id, delta, source, balance, created_at "
                "FROM credit_log "
                "WHERE created_at > ? AND ABS(delta) >= ? "
                "AND char_id > 0 "
    ...
    async def get_farming_alerts(
        self, hourly_threshold: int = 5000,
        sustained_hours: int = 2, lookback_seconds: int = 14400,
    ) -> list:
        """Return characters earning above hourly_threshold for
        sustained_hours or more in the last lookback_seconds (default
        4h).

        Algorithm: bucket positive-delta credit_log rows by char_id and
        hour, then return chars who have >= sustained_hours buckets at
        or above the hourly threshold.

        Returns list of {char_id, hours_over_threshold, peak_hour_total,
        total_in_window} dicts, sorted by total_in_window descending.
        """
        ...
        try:
            rows = await self._db.execute_fetchall(
                ...
                "FROM credit_log "
                "WHERE delta > 0 AND char_id > 0 AND created_at > ? "
            ...
            if not rows:
                return []
            # Aggregate per char: count qualifying hours, peak hour, total
            ...
            for r in rows:
                ...
                by_char[cid]["hours_over_threshold"] += 1
                by_char[cid]["peak_hour_total"] = max(
                    by_char[cid]["peak_hour_total"], ht,
                )
                by_char[cid]["total_in_window"] += ht
            qualifying = [
                v for v in by_char.values()
                if v["hours_over_threshold"] >= sustained_hours
            ]
            qualifying.sort(key=lambda v: v["total_in_window"], reverse=True)
            return qualifying
        except Exception:
    ...
    async def get_inflation_metrics(
        self, seconds: int = 86400,
    ) -> dict:
        """Return inflation snapshot: net flow vs total credits in
        circulation. Net flow > 20% of circulation in a day is the
        warning threshold.

        Returns:
            {
                'net_flow':     int,   # signed: positive = inflation
                'circulation':  int,   # sum of all character.credits
                'flow_pct':     float, # net_flow / circulation (0.0-1.0+)
                'window_seconds': int,
            }
        """
        cutoff = time.time() - seconds
        out = {
            "net_flow": 0, "circulation": 0, "flow_pct": 0.0,
            "window_seconds": seconds,
        }
        try:
            # Net flow over the window (faucets to chars - sinks from chars)
            flow_rows = await self._db.execute_fetchall(
                "SELECT SUM(delta) AS net FROM credit_log "
                "WHERE char_id > 0 AND created_at > ?",
                (cutoff,),
            )
            net = int((flow_rows[0]["net"] or 0) if flow_rows else 0)
            # Total circulation = sum of all character credits
            circ_rows = await self._db.execute_fetchall(
                "SELECT SUM(credits) AS total FROM characters "
                "WHERE credits > 0",
            )
            circ = int((circ_rows[0]["total"] or 0) if circ_rows else 0)
            out["net_flow"] = net
            out["circulation"] = circ
            if circ > 0:
                out["flow_pct"] = net / circ
            return out
        except Exception:
