...
Commands:
  ...
  +quest/accept <id>   — accept a personal quest
  +quest/complete <id> — mark a quest complete
  +quest/abandon <id>  — abandon a personal quest
  @narrative ...       — admin: status / view / update / reset / log

The run-on smashes (questaccept/questcomplete/questabandon) were DELETED in the
command-syntax rework Drop 2; their command classes survive only as switch
implementations dispatched by the +quest umbrella (QuestCommand).
"""
import logging
...
class QuestsCommand(BaseCommand):
    ...
    help_text = (
        ...
        "  +quest/accept <id>    — accept a pending quest\n"
        "  +quest/complete <id>  — mark a quest complete\n"
...
class QuestAcceptCommand(BaseCommand):
    # Switch-impl only — dispatched by the +quest umbrella as +quest/accept.
    ...
    async def execute(self, ctx: CommandContext):
        ...
        if not arg.isdigit():
            await ctx.session.send_line("  Usage: +quest/accept <quest id>")
            return
...
class QuestCompleteCommand(BaseCommand):
    # Switch-impl only — dispatched by the +quest umbrella as +quest/complete.
    # Not registered standalone (command-syntax rework Drop 2).
    key = "questcomplete"
    ...
    async def execute(self, ctx: CommandContext):
        ...
        if not arg.isdigit():
            await ctx.session.send_line("  Usage: +quest/complete <quest id>")
            return
...
class QuestAbandonCommand(BaseCommand):
    # Switch-impl only — dispatched by the +quest umbrella as +quest/abandon.
    # Not registered standalone (command-syntax rework Drop 2).
    key = "questabandon"
    ...
    async def execute(self, ctx: CommandContext):
        ...
        if not arg.isdigit():
            await ctx.session.send_line("  Usage: +quest/abandon <quest id>")
            return
...
_QUEST_ALIAS_TO_SWITCH: dict[str, str] = {
    ...
    "personalquests": "list",
    "list":           "list",
    # accept  (run-on 'questaccept' DELETED — Drop 2; canonical = +quest/accept)
    "acceptquest":    "accept",
    "pqaccept":       "accept",
    "accept":         "accept",
    # complete  (run-on 'questcomplete' DELETED — Drop 2)
    "finishquest":    "complete",
    "pqcomplete":     "complete",
    "completequest":  "complete",
    "complete":       "complete",
    # abandon  (run-on 'questabandon' DELETED — Drop 2)
...
class QuestCommand(BaseCommand):
    ...
    help_text = (
        ...
        "'+quest/complete', '+quest/abandon'. Type 'help +quest' for the "
        "full reference."
    )
...
def register_narrative_commands(registry):
    # The accept/complete/abandon verbs are reachable ONLY via the +quest
    # umbrella's switch dispatch (+quest/accept|complete|abandon) — the run-on
    # standalone keys (questaccept/...) were deleted in the command-syntax
    # rework Drop 2, so those classes are no longer registered standalone.
