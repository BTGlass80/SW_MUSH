...
class SurveyCommand(BaseCommand):
    ...
    async def execute(self, ctx: CommandContext):
        ...
        room_type = (room or {}).get("type", "city")
        ...
        resources = get_survey_resources(room_type, quality)
        if not resources:
...
class ExperimentCommand(BaseCommand):
    """Attempt an experimental craft for higher quality."""
    key = "experiment"
    aliases = ["exp"]
    help_text = (
        "Attempt an experimental craft for higher quality (harder check).\n"
        "Usage: experiment <schematic name>"
    )
    ...
    async def execute(self, ctx: CommandContext):
        ...
        if not ctx.args:
            await ctx.session.send_line(
                "  Usage: experiment <schematic name>  (see 'schematics')"
            )
            return
        ...
        # Experimental: +5 difficulty, but success margin gives +20 quality
        skill = schematic.get("skill", "repair")
        difficulty = schematic.get("difficulty", 10) + 5
        try:
            result = _skill_check(char, skill, difficulty)
            quality_base = survey_quality_from_margin(result.margin) + (
                20 if result.success else 0
            )
            roll_note = (
