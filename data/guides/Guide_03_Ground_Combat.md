...
**The trickle pays zero Character Points.** Advancement is roleplay- and time-gated (the weekly CP cap; see [CP Progression](#/guide/cp-progression)). Grinding mobs can *never* buy skill growth — the credits and prestige it pays don't feed the CP economy. There is exactly one early-game exception, and it lives *outside* the trickle: the first few NPC kills your character *ever* lands in combat each grant a one-time **+1 CP** "first blood" bonus (default five kills, set by `combat.early_cp_kill_cap`, then sealed for good — see [CP Progression](#/guide/cp-progression) §6 for the exact mechanic). It funds roughly one early die-step so an action-first newcomer gets a visible first-session win; past those opening kills, combat goes back to paying zero CP forever. Don't grind expecting to "level up" — grind because the fight was fun and the prestige is yours.
