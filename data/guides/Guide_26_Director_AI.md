...
- **Staged operations.** The Cult of the Hollow Sun (Tatooine's deep desert), the Ember Court (the Geonosis wastes), the Ashen Hand (the Coruscant underworld), and the Iron Veil (the Kuat shipyards) run as real location scenarios. Here `rally` is your **locator** — it names the live **site** and the current stage. You travel to that site and `investigate` it to fight through the operation stage by stage: assault the cult's stronghold (waves of enemies), then work the objective (slice a terminal, turn an informant, splice a relay), then bring down the leader. `rally strike` from across the galaxy just points you to the site — the gameplay is *there*, hands-on, not a slogan you repeat from afar.
- **Galaxy-wide menace cults.** The Drowned Choir (Nar Shaddaa) is fought as a coordinated push from wherever you are. `rally strike` records your contribution — the game rolls your **best pool across playstyles** (a soldier swings, a slicer disrupts their operations, a face rallies civilians against them, a Jedi pushes back the dark side), so every kind of character can help. Each character lands one counted strike per ~10 minutes, and the meter moves on the **community's** total effort — a win is a group achievement, never one person macroing a counter to zero.
