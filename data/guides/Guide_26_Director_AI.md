...
Every cult uprising now runs as a **staged operation**: the Cult of the Hollow Sun (Tatooine's deep desert), the Ember Court (the Geonosis wastes), the Ashen Hand (the Coruscant underworld), the Iron Veil (the Kuat shipyards), and the Drowned Choir (Nar Shaddaa's flooded sub-levels) all run as real location scenarios — no cult is left on the old galaxy-wide menace-counter path. Here `rally` is your **locator** — it names the live **site** and the current stage. You travel to that site and `investigate` it to fight through the operation stage by stage: assault the cult's stronghold (waves of enemies), then work the objective (slice a terminal, turn an informant, splice a relay, trace a patron's ledger), then bring down the leader. `rally strike` from across the galaxy just points you to the site — the gameplay is *there*, hands-on, not a slogan you repeat from afar. (Each stage's objective still honors a **best pool across playstyles** — a soldier swings, a slicer disrupts, a face talks a target down, a Jedi pushes back the dark side — so every kind of character can help once they reach the site.)

**The reward is Republic reputation, a rout bounty, and a relic — in three tiers.** Break a cult and *everyone* who landed at least one counted strike gains **Republic reputation** — the show-up reward, scaled by your share of the effort (roughly 3 for a single strike, up to 15 for the largest contributor). The players who carried a real share of the fight earn two things more: a commemorative status flag marking that they helped rout that cult, and a **credit bounty** for the rout. The status flag and the bounty go to the *same people* — the ones who did the heavy lifting, not everyone who threw a single strike. And the single top contributor takes home a one-off **cult relic** — a rare, multi-stage win is worth showing up for. A loss costs you nothing — an uprising is an opportunity, never a penalty.
