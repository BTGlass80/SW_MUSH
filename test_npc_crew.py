...
def test_archetype_to_role():
    ...
    assert _best_role_for_archetype("scout") == "pilot"  # scout appears in pilot archetypes first
    assert _best_role_for_archetype("smuggler") == "pilot"  # smuggler appears in pilot archetypes first
    assert _best_role_for_archetype("bounty_hunter") == "gunner"
    # Unknown archetype should return fallback
...
def test_resolve_npc_skill():
    ...
    sr = SkillRegistry()
    import os
    skills_path = os.path.join(os.path.dirname(os.path.dirname(__file__)),
                                "data", "skills.yaml")
    if os.path.exists(skills_path):
        sr.load_file(skills_path)
    else:
        # Manual fallback for test
        skills_path = "/mnt/project/data/skills.yaml"
        if os.path.exists(skills_path):
            sr.load_file(skills_path)

    pool = resolve_npc_skill(npc_row, "starship gunnery", sr)
    # mechanical 3D+2 + gunnery 1D+2 = 5D+1 (14+5=19 pips, but DicePool adds)
    # DicePool addition: 3D+2 is (3,2), 1D+2 is (1,2) → (4,4) → normalizes to 5D+1
...
def test_crew_json_helpers():
    ...
    assert 13 in ids
    assert 15 in ids
    # Player IDs should NOT be in the NPC list
    ...
    crew = set_npc_station(crew, "pilot", 15)  # engineer also as pilot (for test)
...
def test_crew_json_parse():
    ...
    ship1 = {"crew": '{"pilot": 1}'}
    assert get_crew_json(ship1)["pilot"] == 1

    ship2 = {"crew": {"pilot": 2}}
    assert get_crew_json(ship2)["pilot"] == 2

    ship3 = {"crew": None}
    assert get_crew_json(ship3) == {}

    ship4 = {"crew": "invalid json{"}
    assert get_crew_json(ship4) == {}

    ship5 = {}
    assert get_crew_json(ship5) == {}
...
def test_weighted_choice():
    """Weighted choice produces valid keys and respects weights roughly."""
    weights = {"a": 0.9, "b": 0.1}
    results = [_weighted_choice(weights) for _ in range(100)]
    assert "a" in results  # Should appear frequently
