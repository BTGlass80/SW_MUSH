# -*- coding: utf-8 -*-
"""
tests/test_generalized_questline_warrens_toll.py — T3.24 generalized
quest expansion, eighth slice.

Proves the EIGHTH accessible (non-t5, non-tutorial) `kind: questline`
chain — "The Warrens Toll" (nar_shaddaa_warrens_toll) — is shipped
correctly and walks start->graduation through the PRODUCTION dispatcher,
the same hooks the live parser calls. Like the first seven slices (The
Ghost Shipment, The Crooked Wheel, The Lost Courier, The Skimmed Line,
The Dust-Sick, The False Provenance, The Forged Notice) it reuses the
...
Distinct from the prior seven:
  * exercises an EIGHTH distinct skill spread — COMMAND + DEMOLITIONS (vs
    search+streetwise / investigation+gambling / persuasion+sneak /
    security+bargain / first_aid+survival / value+con / forgery+
    bureaucracy) — so the eight accessible questlines each teach distinct
    play, and this is the first to lead with the leader's nerve (rallying
    frightened people to act together under threat) and the breacher's
    hand (reading and defusing a live charge);
  * a THIRD, deepest face of Nar Shaddaa — the Warrens, the moon's lowest
    inhabitable levels (warrens_squat_block / warrens_scavenger_den /
    warrens_reactor_core / warrens_collapsed_plaza), reusing NONE of the
    rooms of the two prior Nar Shaddaa arcs (the Ghost Shipment's
    burning_deck_cantina / nar_shaddaa_bay_aurek / floating_market /
    undercity_depths, or the Forged Notice's upper_dock_observation /
    westport_arrivals / smugglers_guild_hall / enforcer_alley);
  * a disaster-prevention / community-defense story shape — break a
    Hutt-space "reactor-safety toll" racket before its planted charge
    breaches the Warrens reactor: rally a frightened level, defuse the
    charge, and stop the enforcer who comes to set another — not a
    smuggling-trace, a racket-bust, a kidnap-rescue, an industrial-skim,
    a frontier rescue, an art-fraud caper, or a counter-forgery.
...
QUESTLINE_ID = "nar_shaddaa_warrens_toll"
ACHIEVEMENT_KEY = "warrens_toll_broken"
GIVER_NPC = "Vessa Tial"
ANTAGONIST_NPC = "Tarko Venn"
ENEMY_TEMPLATE = "warrens_toll_enforcer"
START_ROOM = "warrens_squat_block"
GIVER_ROOM_NAME = "Nar Shaddaa - The Warrens - Squat Block"
NPC_FILE = (PROJECT_ROOT / "data" / "worlds" / "clone_wars"
            / "npcs_drop_generalized_questline_warrens_toll.yaml")
...
class TestWalkthrough(_RealCorpusBase):
    ...
    def test_full_walkthrough_to_graduation(self):
        ...
        # Step 1: talk to Vessa Tial (the squat-block organizer)
        _run(on_talk_to_npc(db, char, GIVER_NPC))
        self.assertEqual(_qstate(char).get("step"), 2)

        # Step 2: rally the level (command)
        _run(on_skill_check_passed(db, char, "command", True,
                                   difficulty=11))
        self.assertEqual(_qstate(char).get("step"), 3)

        # Step 3: defuse the charge on the reactor fins (demolitions)
        _run(on_skill_check_passed(db, char, "demolitions", True,
                                   difficulty=13))
        self.assertEqual(_qstate(char).get("step"), 4)

        # Step 4: stop Tarko Venn in the Collapsed Plaza (combat_won, count 1)
        _run(on_combat_won(db, char, ENEMY_TEMPLATE, 1))
        self.assertEqual(_qstate(char).get("step"), 5)

        # Step 5: return to Vessa -> graduate
        _run(on_talk_to_npc(db, char, GIVER_NPC))
    ...
    def test_skill_failure_does_not_advance(self):
        ...
        _run(on_skill_check_passed(db, char, "command", False,
                                   difficulty=11))
        self.assertEqual(_qstate(char).get("step"), 2)  # no advance

    def test_wrong_skill_does_not_advance(self):
        # Step 2 gates on command; a passing demolitions check (this
...
class TestReachabilityBits(_RealCorpusBase):
    ...
    def test_skill_spread_is_command_and_demolitions(self):
        # The eighth distinct spread: the two skill_check_passed steps gate
        # on command then demolitions (no prior accessible questline does).
        ql = self._questline()
        skills = [(s.completion or {}).get("skill") for s in ql.steps
                  if (s.completion or {}).get("type") == "skill_check_passed"]
        self.assertEqual(skills, ["command", "demolitions"])

    def test_reuses_no_prior_nar_shaddaa_arc_rooms(self):
        # Three questlines are now on Nar Shaddaa; this one deliberately runs
        # the deepest, distinct face of the moon, so it must share no step
        # rooms with the Ghost Shipment OR the Forged Notice (else the
        # "different face" claim is false).
        prior_ns_rooms = {
            # Ghost Shipment
        ...
        self.assertFalse(mine & prior_ns_rooms,
                         f"overlaps prior Nar Shaddaa arc rooms: "
                         f"{mine & prior_ns_rooms}")
...
class TestNpcs(_RealCorpusBase):
    ...
    def test_giver_present_in_start_room(self):
        ...
        # Display name of warrens_squat_block.
    ...
    def test_npc_file_wired_into_era(self):
        ...
        self.assertIn(
            "npcs_drop_generalized_questline_warrens_toll.yaml", npc_refs)
