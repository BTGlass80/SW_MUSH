# -*- coding: utf-8 -*-
"""
tests/test_drop_mob_grind_tatooine_authority_zones.py

Verifies the Tatooine Authority & Government Zones hostile-mob grind batch
(data/worlds/clone_wars/npcs_drop_mob_grind_tatooine_authority_zones.yaml).

Six NPCs seeded across six previously uncovered Tatooine authority/government
rooms — the Republic checkpoint, the scavenger market, the government quarter,
the spaceport customs office, the control tower, and the militia headquarters.
...
MOB_FILE = os.path.join(CW_DIR, "npcs_drop_mob_grind_tatooine_authority_zones.yaml")
...
class TestEraManifestRef(unittest.TestCase):
    def test_file_in_era_yaml(self):
        ...
        ref_name = "npcs_drop_mob_grind_tatooine_authority_zones.yaml"
        self.assertIn(
            ref_name, npcs_refs,
