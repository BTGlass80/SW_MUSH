# -*- coding: utf-8 -*-
"""
tests/test_drop_mob_grind_kuat_expansion.py

Verifies the Kuat Expansion hostile-mob grind batch
(data/worlds/clone_wars/npcs_drop_mob_grind_kuat_expansion.yaml).

Six NPCs seeded across six previously uncovered Kuat rooms spanning the
kdy_orbital_ring and kuat_city_embassy zones. All must satisfy
...
MOB_FILE = os.path.join(CW_DIR, "npcs_drop_mob_grind_kuat_expansion.yaml")
...
EXPECTED_ROOMS = {
    ...
    "Kuat Drive Yards - House Andrim Suite",
    "Kuat Drive Yards - House Purkis Suite",
    "Kuat - Military Space Station",
    "Kuat City - Mercantile Forum",
    "Kuat City - Estate District",
}
...
class TestEraManifestRef(unittest.TestCase):
    def test_file_in_era_yaml(self):
        ...
        ref_name = "npcs_drop_mob_grind_kuat_expansion.yaml"
        self.assertIn(
            ref_name, npcs_refs,
