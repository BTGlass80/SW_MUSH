# -*- coding: utf-8 -*-
"""
tests/test_drop_ambient_geonosis_hive_surface.py

Static-invariant tests for the Geonosis Hive & Surface ambient NPC batch
(data/worlds/clone_wars/npcs_drop_ambient_geonosis_hive_surface.yaml).

Four non-hostile flavor NPCs across four Geonosis rooms:
  Smuggler's Roost Flophouse        x1  (Vez Garra / Weequay -- named in room desc)
  Hive Interior Market              x1  (Hive Mineral Trader Kass-ett / Geonosian)
  Stalgasin Surface Offworld Qtr    x1  (CIS Engineer Ralan Theis / Human)
  Petranaki Arena - Stands          x1  (CIS Tactical Observer Drev Sollum / Human)
...
Verifies:
  ...
  7. Room references resolve to real Geonosis rooms
...
YAML_PATH = os.path.join(CW_DIR, "npcs_drop_ambient_geonosis_hive_surface.yaml")
ERA_PATH = os.path.join(CW_DIR, "era.yaml")
GEONOSIS_PATH = os.path.join(CW_DIR, "planets", "geonosis.yaml")

EXPECTED_NPC_COUNT = 4

EXPECTED_NPCS = {
    "Vez Garra",
    "Hive Mineral Trader Kass-ett",
    "CIS Engineer Ralan Theis",
    "CIS Tactical Observer Drev Sollum",
}

EXPECTED_ROOMS = {
    "Geonosis - Stalgasin Surface - Smuggler's Roost Flophouse",
    "Geonosis - Hive Interior Market",
    "Geonosis - Stalgasin Surface - Offworld Quarter",
    "Geonosis - Petranaki Arena - Stands",
}
...
def _geonosis_room_names():
    data = _load_yaml(GEONOSIS_PATH)
...
class TestRoomReferences(unittest.TestCase):
    def setUp(self):
        data = _load_yaml(YAML_PATH)
        self.npcs = data["npcs"]
        self.geonosis_rooms = _geonosis_room_names()

    def test_rooms_resolve_in_geonosis(self):
        for npc in self.npcs:
            room = npc.get("room", "")
            self.assertIn(room, self.geonosis_rooms,
                          f"{npc['name']}: room '{room}' not found in geonosis.yaml")
...
class TestEraRegistration(unittest.TestCase):
    def test_yaml_in_era_npcs_list(self):
        ...
        self.assertIn(
            "npcs_drop_ambient_geonosis_hive_surface.yaml",
            names,
            "npcs_drop_ambient_geonosis_hive_surface.yaml not in era.yaml content_refs.npcs",
        )
