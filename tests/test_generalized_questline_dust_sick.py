# -*- coding: utf-8 -*-
"""
tests/test_generalized_questline_dust_sick.py — T3.24 generalized
quest expansion, fifth slice.

Proves the FIFTH accessible (non-t5, non-tutorial) `kind: questline`
chain — "The Dust-Sick" (jundland_dust_sick) — is shipped correctly and
walks start->graduation through the PRODUCTION dispatcher, the same
hooks the live parser calls. Like the first four slices (The Ghost
Shipment, The Crooked Wheel, The Lost Courier, The Skimmed Line) it
reuses the live questline engine (active_questline slot, the existing
...
Distinct from the prior four:
  * the FIRST accessible questline set in the WILDERNESS — the Tatooine
    deep desert (the City Outskirts scavenger market and the Jundland
    Wastes), not a city interior like Nar Shaddaa (Ghost Shipment) /
    Mos Eisley (Crooked Wheel) / Coruscant lower city (Lost Courier) /
    the Kuat ring (Skimmed Line);
  * exercises a FIFTH distinct skill spread — FIRST_AID + SURVIVAL (vs
    search+streetwise / investigation+gambling / persuasion+sneak /
    security+bargain) — so the five accessible questlines each teach
    distinct play, and this is the first to lead with the medical and
    wilderness skills;
  * a save-the-stricken frontier-rescue story shape, not a racket-bust,
    a smuggling-trace, a kidnap-rescue, or an industrial-theft.
...
QUESTLINE_ID = "jundland_dust_sick"
ACHIEVEMENT_KEY = "dust_sick_recovered"
GIVER_NPC = "Soree Bann"
ANTAGONIST_NPC = "Vohrek Sarn"
ENEMY_TEMPLATE = "jundland_dust_scavenger_boss"
START_ROOM = "outskirts_scavenger_market"
GIVER_ROOM_NAME = "City Outskirts - Scavenger Market"
NPC_FILE = (PROJECT_ROOT / "data" / "worlds" / "clone_wars"
            / "npcs_drop_generalized_questline_dust_sick.yaml")
...
class TestWalkthrough(_RealCorpusBase):
    ...
    def test_full_walkthrough_to_graduation(self):
        ...
        # Step 1: talk to Soree Bann (the homesteader)
        _run(on_talk_to_npc(db, char, GIVER_NPC))
        self.assertEqual(_qstate(char).get("step"), 2)

        # Step 2: stabilize the fevered boy (first_aid)
        _run(on_skill_check_passed(db, char, "first_aid", True,
                                   difficulty=10))
        self.assertEqual(_qstate(char).get("step"), 3)

        # Step 3: read the desert for a way home (survival)
        _run(on_skill_check_passed(db, char, "survival", True,
                                   difficulty=12))
        self.assertEqual(_qstate(char).get("step"), 4)

        # Step 4: drive off Vohrek Sarn (combat_won, count 1)
        _run(on_combat_won(db, char, ENEMY_TEMPLATE, 1))
        self.assertEqual(_qstate(char).get("step"), 5)

        # Step 5: return to Soree -> graduate
        _run(on_talk_to_npc(db, char, GIVER_NPC))
    ...
    def test_skill_failure_does_not_advance(self):
        ...
        _run(on_skill_check_passed(db, char, "first_aid", False,
                                   difficulty=10))
        self.assertEqual(_qstate(char).get("step"), 2)  # no advance

    def test_wrong_skill_does_not_advance(self):
        # Step 2 gates on first_aid; a passing survival check (this
        # questline's OWN step-3 skill) must NOT advance step 2 — the
...
class TestReachabilityBits(_RealCorpusBase):
    ...
    def test_skill_spread_is_first_aid_and_survival(self):
        # The fifth distinct spread: the two skill_check_passed steps gate
        # on first_aid then survival (no prior accessible questline does).
        ql = self._questline()
        skills = [(s.completion or {}).get("skill") for s in ql.steps
                  if (s.completion or {}).get("type") == "skill_check_passed"]
        self.assertEqual(skills, ["first_aid", "survival"])
...
class TestNpcs(_RealCorpusBase):
    ...
    def test_giver_present_in_start_room(self):
        ...
        # Display name of outskirts_scavenger_market.
    ...
    def test_npc_file_wired_into_era(self):
        ...
        self.assertIn(
            "npcs_drop_generalized_questline_dust_sick.yaml", npc_refs)
