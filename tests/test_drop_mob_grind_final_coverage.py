# -*- coding: utf-8 -*-
"""
tests/test_drop_mob_grind_final_coverage.py
...
MOB_FILE = os.path.join(CW_DIR, "npcs_drop_mob_grind_final_coverage.yaml")
...
class TestEraManifestRef(unittest.TestCase):
    def test_file_in_era_yaml(self):
        ...
        ref_name = "npcs_drop_mob_grind_final_coverage.yaml"
        self.assertIn(
            ref_name, npcs_refs,
...
class TestCoverageComplete(unittest.TestCase):
    def test_all_target_rooms_have_npcs(self):
        data = _load_yaml(MOB_FILE)
        rooms_present = {npc.get("room") for npc in data["npcs"]}
        for expected_room in _EXPECTED_ROOMS:
            self.assertIn(
                expected_room, rooms_present,
                f"Expected room '{expected_room}' has no NPC in the final coverage batch",
            )
