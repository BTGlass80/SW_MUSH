...
class TestAvailableMissionsForChar:
    ...
    def test_faction_mission_hidden_when_rep_below_threshold(self):
        ...
        db.get_organization = AsyncMock(return_value={"id": 10, "code": "republic"})
        db.get_membership = AsyncMock(return_value=None)
        # Attributes-based rep lookup returns 0

        visible = self._run(available_missions_for_char(char, db, [m]))
        assert len(visible) == 0, "Republic mission should be filtered out for 0-rep char"
    ...
    def test_faction_mission_visible_when_rep_meets_threshold(self):
        ...
        char = self._fake_char()
        db = MagicMock()
        db.get_organization = AsyncMock(return_value={"id": 11, "code": "cis"})
    ...
    def test_faction_mission_visible_when_rep_exceeds_threshold(self):
        ...
        char = self._fake_char()
        db = MagicMock()
        db.get_organization = AsyncMock(return_value={"id": 12, "code": "hutt_cartel"})
    ...
    def test_mixed_board_filters_correctly(self):
        ...
        republic_missions = [generate_faction_mission("republic") for _ in range(2)]
        cis_missions = [generate_faction_mission("cis") for _ in range(2)]
        hutt_missions = [generate_faction_mission("hutt_cartel") for _ in range(2)]
        board = open_missions + republic_missions + cis_missions + hutt_missions
        assert len(board) == 8
        ...
        # - republic: 30 (qualified)
        # - cis: 10 (NOT qualified, threshold is 25)
        # - hutt_cartel: 0 (NOT qualified)
        orgs_by_code = {
            "republic": {"id": 10, "code": "republic"},
            "cis":  {"id": 11, "code": "cis"},
            "hutt_cartel":   {"id": 12, "code": "hutt_cartel"},
        }
        mems_by_org = {
            10: {"rep_score": 30},
            11: {"rep_score": 10},
    ...
    def test_rep_cache_avoids_redundant_db_calls(self):
        ...
        republic_missions = [generate_faction_mission("republic") for _ in range(5)]
        ...
        self._run(available_missions_for_char(char, db, republic_missions))

        # Even though there are 5 republic missions, the rep lookup should only
...
class TestFactionMissionConfigShape:
    ...
    def test_all_documented_factions_present(self):
        """CW faction config names republic/cis/hutt/bh_guild."""
        from engine.missions import FACTION_MISSION_CONFIG
        for fc in ("republic", "cis", "jedi_order", "hutt_cartel", "bounty_hunters_guild"):
...
class TestGenerateFactionMissionBehaviour:
    ...
    def test_bh_guild_mission_generates(self):
        """bounty_hunters_guild is a documented faction; S38 only tests three."""
        from engine.missions import generate_faction_mission
        m = generate_faction_mission("bounty_hunters_guild")
        assert m is not None
        assert m.faction_code == "bounty_hunters_guild"
    ...
    def test_accepts_destination_rooms(self):
        ...
        found = False
        for _ in range(20):
            m = generate_faction_mission("republic", destination_rooms=rooms)
    ...
    def test_skill_level_scales_reward(self):
        ...
        import random
        random.seed(42)
        low_rewards = [generate_faction_mission("republic", skill_level=1).reward for _ in range(50)]
        random.seed(42)
        high_rewards = [generate_faction_mission("republic", skill_level=6).reward for _ in range(50)]
