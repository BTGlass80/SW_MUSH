# -*- coding: utf-8 -*-
"""
tests/test_drop_ambient_kuat_industrial_civic.py

Static-invariant tests for the Kuat Industrial & Civic ambient NPC batch
(data/worlds/clone_wars/npcs_drop_ambient_kuat_industrial_civic.yaml).

Six non-hostile flavor NPCs across four KDY/Kuat City rooms:
  The Pressure Drop Cantina     x2  (Veela Torsk / Jarec Mave -- shipwrights)
  Ring Residential Block        x2  (Narra Kessel / Tov Saren -- resident + tech)
  Machine Shops                 x1  (Pressa Velk -- master fabricator)
  Kuat City - Mercantile Forum  x1  (Heset Andrim -- House Andrim observer)
...
Verifies:
  ...
  7. Room references resolve to real Kuat rooms
...
YAML_PATH = os.path.join(CW_DIR, "npcs_drop_ambient_kuat_industrial_civic.yaml")
ERA_PATH = os.path.join(CW_DIR, "era.yaml")
KUAT_PATH = os.path.join(CW_DIR, "planets", "kuat.yaml")
...
EXPECTED_ROOMS = {
    "Kuat Drive Yards - The Pressure Drop Cantina",
    "Kuat Drive Yards - Ring Residential Block",
    "Kuat Drive Yards - Machine Shops",
    "Kuat City - Mercantile Forum",
}
...
def _kuat_room_names():
    data = _load_yaml(KUAT_PATH)
...
class TestRoomReferences(unittest.TestCase):
    def setUp(self):
        data = _load_yaml(YAML_PATH)
        self.npcs = data["npcs"]
        self.kuat_rooms = _kuat_room_names()

    def test_rooms_resolve_in_kuat(self):
        for npc in self.npcs:
            room = npc.get("room", "")
            self.assertIn(room, self.kuat_rooms,
                          f"{npc['name']}: room '{room}' not found in kuat.yaml")
...
class TestEraRegistration(unittest.TestCase):
    def test_yaml_in_era_npcs_list(self):
        ...
        self.assertIn(
            "npcs_drop_ambient_kuat_industrial_civic.yaml",
            names,
            "npcs_drop_ambient_kuat_industrial_civic.yaml not in era.yaml content_refs.npcs",
        )
