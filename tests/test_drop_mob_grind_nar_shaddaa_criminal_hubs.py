# -*- coding: utf-8 -*-
"""
tests/test_drop_mob_grind_nar_shaddaa_criminal_hubs.py

Verifies the Nar Shaddaa criminal hubs hostile-mob grind batch
(data/worlds/clone_wars/npcs_drop_mob_grind_nar_shaddaa_criminal_hubs.yaml).
...
MOB_FILE = os.path.join(CW_DIR, "npcs_drop_mob_grind_nar_shaddaa_criminal_hubs.yaml")
...
class TestYamlParses(unittest.TestCase):
    ...
    def test_six_npcs(self):
        data = _load_yaml(MOB_FILE)
        self.assertGreaterEqual(len(data["npcs"]), 4, "Expected at least 4 hostile mobs (was 6; 2 SECURED-dead removed in grind realignment 2026-06-24)")
...
class TestEraManifestRef(unittest.TestCase):
    def test_file_in_era_yaml(self):
        ...
        ref_name = "npcs_drop_mob_grind_nar_shaddaa_criminal_hubs.yaml"
        self.assertIn(
            ref_name, npcs_refs,
