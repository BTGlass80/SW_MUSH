# -*- coding: utf-8 -*-
"""
tests/test_drop_mob_grind_geonosis_stalgasin_market.py

Verifies the Geonosis Stalgasin Surface & Market hostile-mob grind batch
(data/worlds/clone_wars/npcs_drop_mob_grind_geonosis_stalgasin_market.yaml).

Covers 6 previously-uncovered Geonosis rooms (Surface Market x2, CIS
Recruitment Office, Archduke's Audience Hall, Offworld Quarter, Outsider
Compound, Smuggler's Roost Flophouse).

All seven NPCs must satisfy engine/hunting_rewards.is_huntable_mob():
...
MOB_FILE = os.path.join(CW_DIR, "npcs_drop_mob_grind_geonosis_stalgasin_market.yaml")
...
class TestEraManifestRef(unittest.TestCase):
    def test_file_in_era_yaml(self):
        ...
        ref_name = "npcs_drop_mob_grind_geonosis_stalgasin_market.yaml"
        self.assertIn(
            ref_name, npcs_refs,
