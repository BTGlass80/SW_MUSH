# -*- coding: utf-8 -*-
"""
tests/test_generalized_questline_crooked_wheel.py — T3.24 generalized
quest expansion, second slice.

Proves the SECOND accessible (non-t5, non-tutorial) `kind: questline`
chain — "The Crooked Wheel" (mos_eisley_crooked_wheel) — is shipped
correctly and walks start->graduation through the PRODUCTION dispatcher,
the same hooks the live parser calls. Like the first slice (The Ghost
Shipment) it reuses the live questline engine (active_questline slot, the
existing event types, the four reward funnels) with NO new engine code,
per quest_expansion_postlaunch_path_v1.md.

Distinct from the Ghost Shipment (test_generalized_questline_ghost_shipment):
this one is set on Tatooine/Mos Eisley (not Nar Shaddaa) and exercises a
DIFFERENT skill spread (investigation + gambling, vs search + streetwise),
so the two accessible questlines teach distinct play. The test pins the
accessibility (chargen_complete, no rep gate), the modest reward band
(below the tuned rep ceiling), the registered+linked achievement, real
room slugs, the giver NPC presence, and the combat foil's
...
QUESTLINE_ID = "mos_eisley_crooked_wheel"
ACHIEVEMENT_KEY = "crooked_wheel_busted"
GIVER_NPC = "Dorae Vint"
ANTAGONIST_NPC = "Gorvahn Brusk"
ENEMY_TEMPLATE = "mos_eisley_crooked_wheel_enforcer"
START_ROOM = "market_place_geps_grill"
NPC_FILE = (PROJECT_ROOT / "data" / "worlds" / "clone_wars"
            / "npcs_drop_generalized_questline_crooked_wheel.yaml")
...
def _char(attrs: dict = None) -> dict:
    ...
    return {
        "id": 31, "name": "Freelancer PC", "room_id": 100,
...
class TestWalkthrough(_RealCorpusBase):
    ...
    def test_skill_failure_does_not_advance(self):
        ...
        _run(on_skill_check_passed(db, char, "investigation", False,
                                   difficulty=10))
        self.assertEqual(_qstate(char).get("step"), 2)  # no advance

    def test_wrong_skill_does_not_advance(self):
        # Step 2 gates on investigation; a passing streetwise check (the
        # Ghost Shipment's skill) must NOT advance this questline.
        from engine.chain_events import (
            start_questline, on_talk_to_npc, on_skill_check_passed,
        )
        char = _char()
        ...
        _run(start_questline(db, char, QUESTLINE_ID))
        _run(on_talk_to_npc(db, char, GIVER_NPC))  # ->2
        _run(on_skill_check_passed(db, char, "streetwise", True,
                                   difficulty=10))
        self.assertEqual(_qstate(char).get("step"), 2)  # no advance
...
class TestNpcs(_RealCorpusBase):
    ...
    def test_giver_present_in_start_room(self):
        ...
        # Display name of market_place_geps_grill.
        self.assertEqual(giver["room"], "Market Place - Gep's Grill")
    ...
    def test_npc_file_wired_into_era(self):
        ...
        self.assertIn(
            "npcs_drop_generalized_questline_crooked_wheel.yaml", npc_refs)
