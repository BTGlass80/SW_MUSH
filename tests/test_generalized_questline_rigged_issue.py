# -*- coding: utf-8 -*-
"""
tests/test_generalized_questline_rigged_issue.py — T3.24 generalized
quest expansion, seventeenth slice.

Proves the SEVENTEENTH accessible (non-t5, non-tutorial) `kind: questline`
chain — "The Rigged Issue" (tatooine_rigged_issue) — is shipped correctly and
walks start->graduation through the PRODUCTION dispatcher, the same hooks the
live parser calls. Like the first sixteen slices (The Ghost Shipment, The
...
Haul, The Twisted Word, The Wasting Ward) it reuses the live questline engine
(active_questline slot, the existing event types, the four reward funnels) with
NO new engine code, per quest_expansion_postlaunch_path_v1.md.

Firsts for the accessible-questline arc:
  * a SEVENTEENTH distinct skill spread — BLASTER REPAIR + ARMOR REPAIR (both
    TECHNICAL) — neither of which any prior accessible questline uses (vs
    ...
    medicine+scholar). The FIRST accessible questline to reward the GUNSMITH /
    ARMORER (weaponsmith) build — strip a "certified" sidearm and read the rigged
    action, then put a plate of "issue" armor under the light and read the
    counterfeit cast in it. It is mechanically DISTINCT from the two prior
    repair-skill arcs, BOTH of which work MACHINES not personal arms: The
    Condemned Hull rewards SPACE TRANSPORT REPAIR + ASTROGATION (a starship
    engineer-navigator) and The Long Haul rewards GROUND VEHICLE REPAIR + GROUND
    VEHICLE OPERATION (a wheelman-mechanic). This is the armorer's bench — a
    blaster and a breastplate in the hand, not a hull or a hauler;
  * the FIFTH questline set on TATOOINE and the first on the CIVIC-COMPOUND face
    (tatooine_militia_hq / regional_government_offices / police_station_main /
    mos_eisley_government_quarter — all zone tatooine_civic), every room of which
    is FRESH to the entire chain corpus — reusing NONE of The Crooked Wheel's
    market district, The Dust-Sick's Jundland, The Sabotaged Run's speeder
    circuit, or The Long Haul's freight margins. The bored clone garrison and
    twice-a-week Republic liaison the militia_hq/police_station room descriptions
    name stay distant background — the garrison here for the war, not a militia's
    bad guns — so the war stays offstage the way every prior accessible arc keeps
    the Republic/CIS conflict offstage;
  * a foil who carries the proven blaster_pistol of the ranged foils, squarely
    in the proven beatable band (NOT the offset melee weapon The Condemned Hull's
    foil carried).

The story shape is new too — breaking a COUNTERFEIT / SABOTAGED-ARMS racket (an
arms-fraud supply scam; no prior accessible arc touches one — The Condemned Hull
falsely CONDEMNS a good ship to seize it, this floods a militia with BAD arms and
skims the good): a licensed outfitter holding the Tatooine Planetary Militia's
small-arms contract substitutes the genuine issue for counterfeits (blasters cut
to a cheaper, hotter spec that jam after a magazine, "ceramic-faced" armor that is
forged-marked pot-metal), pocketing the difference and fencing the real arms
offworld, and a patrol rider is dead of a blaster that quit on him with a raider
on his back, so strip one of the "certified" replacement blasters and prove it
built to fail (blaster repair), prove the dead rider's "issue" plate a
forged-marked counterfeit (armor repair), stand off the enforcer the outfitter
sends when the proof starts to walk (combat_won), and get the proof to the
prefect's weapons-licensing desk. It carries a real combat climax (step 4), with
...
registered+linked achievement, real Tatooine room slugs (all in the
tatooine_civic zone), the giver + foil NPCs, the combat-climax structure, the
all-Tatooine-rooms + every-room-fresh-to-corpus claim, the seventeenth-distinct-
spread + both-Technical armorer claim + distinctness from the two prior repair
arcs, and that the authored spread skills resolve to a trained character's real
...
QUESTLINE_ID = "tatooine_rigged_issue"
ACHIEVEMENT_KEY = "rigged_issue_cleared"
GIVER_NPC = "Senni Vrook"
ANTAGONIST_NPC = "Vurr Magh"
ENEMY_TEMPLATE = "rigged_issue_enforcer"
START_ROOM = "tatooine_militia_hq"
GIVER_ROOM_NAME = "Tatooine Militia Headquarters"
ANTAGONIST_ROOM_NAME = "Mos Eisley Street - Government Quarter"
CIVIC_ZONE = "tatooine_civic"
NPC_FILE = (PROJECT_ROOT / "data" / "worlds" / "clone_wars"
            / "npcs_drop_generalized_questline_rigged_issue.yaml")

# The seventeenth skill spread, in step order (steps 2/3 are skill_check_passed).
EXPECTED_SKILLS = ["blaster repair", "armor repair"]

# The skill spreads of the prior SIXTEEN accessible questlines (each non-combat
# skill that gates a skill_check_passed step). The seventeenth spread must share
# NO skill with any of them — the "seventeenth DISTINCT spread" claim.
PRIOR_SPREAD_SKILLS = {
...
# The two prior REPAIR-skill arcs — both work machines, not personal arms. The
# seventeenth shares no spread skill with either (the "distinct armorer" claim).
CONDEMNED_HULL_SPREAD = {"space transport repair", "astrogation"}
LONG_HAUL_SPREAD = {"ground vehicle repair", "ground vehicle operation"}
...
def _civic_zone_slugs() -> set:
    return {r.get("slug") for r in _tatooine_rooms()
            if r.get("zone") == CIVIC_ZONE}
...
class TestWalkthrough(_RealCorpusBase):
    ...
    def test_full_walkthrough_to_graduation(self):
        ...
        # Step 1: talk to Senni Vrook (the armorer)
        _run(on_talk_to_npc(db, char, GIVER_NPC))
        self.assertEqual(_qstate(char).get("step"), 2)

        # Step 2: prove the "certified" blaster built to jam (blaster repair)
        _run(on_skill_check_passed(db, char, "blaster repair", True,
                                   difficulty=11))
        self.assertEqual(_qstate(char).get("step"), 3)

        # Step 3: prove the dead rider's plate a forged counterfeit (armor repair)
        _run(on_skill_check_passed(db, char, "armor repair", True,
                                   difficulty=13))
        self.assertEqual(_qstate(char).get("step"), 4)

        # Step 4: stop Vurr Magh in the government quarter (combat_won, count 1)
        _run(on_combat_won(db, char, ENEMY_TEMPLATE, 1))
        self.assertEqual(_qstate(char).get("step"), 5)

        # Step 5: return to Senni Vrook -> graduate
        _run(on_talk_to_npc(db, char, GIVER_NPC))
    ...
    def test_skill_failure_does_not_advance(self):
        ...
        _run(on_skill_check_passed(db, char, "blaster repair", False,
                                   difficulty=11))
        self.assertEqual(_qstate(char).get("step"), 2)  # no advance

    def test_wrong_skill_does_not_advance(self):
        # Step 2 gates on blaster repair; a passing armor repair check (this
...
class TestReachabilityBits(_RealCorpusBase):
    ...
    def test_skill_spread_is_blaster_then_armor_repair(self):
        # The seventeenth distinct spread: the two skill_check_passed steps gate
        # on blaster repair then armor repair (no prior accessible questline uses
        # either).
        ql = self._questline()
    ...
    def test_spread_is_distinct_from_all_prior_arcs(self):
        # The "seventeenth DISTINCT spread" claim: neither spread skill is used
        # by any of the prior sixteen accessible questlines.
        self.assertFalse(
    ...
    def test_spread_is_both_technical_armorer(self):
        # The defining first: the spread is the GUNSMITH / ARMORER build —
        # blaster repair + armor repair, BOTH Technical. Grounded against the
        # live skills.yaml.
        ...
        self.assertIn("blaster repair", technical,
                      "'blaster repair' is not a Technical skill — the armorer "
                      "claim is false")
        self.assertIn("armor repair", technical,
                      "'armor repair' is not a Technical skill — the armorer "
                      "claim is false")

    def test_distinct_from_prior_repair_arcs(self):
        # The differentiation from the two prior repair-skill arcs, both of which
        # work MACHINES: The Condemned Hull (space transport repair + astrogation)
        # and The Long Haul (ground vehicle repair + operation). This works
        # PERSONAL ARMS and shares NO spread skill with either.
        self.assertFalse(
            set(EXPECTED_SKILLS) & CONDEMNED_HULL_SPREAD,
            "The Rigged Issue shares a spread skill with The Condemned Hull — "
            "the 'distinct from the starship engineer' claim is false")
        self.assertFalse(
            set(EXPECTED_SKILLS) & LONG_HAUL_SPREAD,
            "The Rigged Issue shares a spread skill with The Long Haul — the "
            "'distinct from the wheelman-mechanic' claim is false")
    ...
    def test_spread_skills_resolve_to_trained_pools(self):
        ...
        trained = {
            ...
            "skills": json.dumps({"blaster repair": "5D"}),
        }
        untrained = {
            "attributes": json.dumps({"technical": "3D"}),
            "skills": json.dumps({}),
        }
        trained_pool = _get_skill_pool(trained, "blaster repair", reg)
        raw_pool = _get_skill_pool(untrained, "blaster repair", reg)
        # A char who trained Blaster Repair must roll a STRICTLY larger pool than
        # a char rolling raw Technical — proving the authored skill resolves to
        # the trained skill rather than the bare attribute.
        self.assertGreater(
            trained_pool, raw_pool,
            "authored spread skill 'blaster repair' must roll the trained pool, "
            "not raw Technical")

    def test_all_step_rooms_are_tatooine_civic_rooms(self):
        # The "fifth questline set on Tatooine / civic-compound face" claim:
        # every step room (and the drop room) is a real loaded Tatooine room in
        # the tatooine_civic zone.
        tat = _tatooine_room_slugs()
        civic = _civic_zone_slugs()
        ql = self._questline()
        for step in ql.steps:
            self.assertIn(step.location, tat,
                          f"step {step.step} location {step.location!r} is not "
                          f"a Tatooine room — the 'fifth Tatooine arc' claim is "
                          f"false")
            self.assertIn(step.location, civic,
                          f"step {step.step} location {step.location!r} is not "
                          f"in the tatooine_civic zone — the 'civic-compound "
                          f"face' claim is false")
        self.assertIn(ql.graduation.drop_room, civic)
    ...
    def test_every_room_is_fresh_to_the_corpus(self):
        ...
        self.assertFalse(
            overlap,
            f"The Rigged Issue reuses rooms already in the chain corpus "
...
class TestNpcs(_RealCorpusBase):
    ...
    def test_giver_present_in_start_room(self):
        ...
        # Display name of tatooine_militia_hq.
    ...
    def test_antagonist_carries_proven_ranged_weapon(self):
        # Back in the proven beatable band: Vurr Magh carries blaster_pistol
        # (the ranged foils' weapon) so a fresh post-chargen character has a
        # real, winnable fight with no balance flag (NOT the offset melee weapon
        # The Condemned Hull's foil carried).
        ant = self.by_name[ANTAGONIST_NPC]
    ...
    def test_exactly_two_placed_npcs(self):
        ...
        # licensed outfitter, his offworld fence, the substituted "certified"
        # stock and forged proof-marks, the dead patrol rider and his recovered
        # kit, the militia quartermaster, the bored clone garrison and Republic
        # liaison, and the prefect's licensing desk are narrated-only.
        self.assertEqual(len(self.npcs), 2,
                         "The Rigged Issue should place exactly two NPCs "
    ...
    def test_npc_file_wired_into_era(self):
        ...
        self.assertIn(
            "npcs_drop_generalized_questline_rigged_issue.yaml", npc_refs)
