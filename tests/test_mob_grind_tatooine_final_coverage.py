"""
tests/test_mob_grind_tatooine_final_coverage.py

Static invariant tests for the Tatooine Final Coverage hostile mob grind batch
(data/worlds/clone_wars/npcs_drop_mob_grind_tatooine_final_coverage.yaml).

Covers the 6 remaining Tatooine rooms without mob coverage:
  tatooine_market (2):
    Lup's General Store
    Jawa Traders
  tatooine_mos_eisley (2):
    Mos Eisley Inn
...
GRIND_FILE = os.path.join(DATA_DIR, "npcs_drop_mob_grind_tatooine_final_coverage.yaml")
...
def test_rooms_are_expected_target_rooms(grind_data):
    """Every NPC is placed in one of the six target rooms."""
    for npc in grind_data["npcs"]:
        assert npc["room"] in EXPECTED_ROOMS, (
            f"{npc['name']}: room {npc['room']!r} is not a target final-coverage room"
        )
...
def test_era_cleanness(grind_data):
    ...
    for term in ERA_FORBIDDEN:
        assert term not in raw, (
            f"Era violation: {term!r} found in Tatooine final coverage mob grind data"
        )
...
def test_registered_in_era_yaml(era_data):
    ...
    assert "tatooine_final_coverage" in raw, (
        "npcs_drop_mob_grind_tatooine_final_coverage.yaml not registered in era.yaml"
    )
