# -*- coding: utf-8 -*-
"""
tests/test_drop_mob_grind_tatooine_commerce_services.py

Verifies the Tatooine Commerce & Services hostile-mob grind batch
(data/worlds/clone_wars/npcs_drop_mob_grind_tatooine_commerce_services.yaml).

Six NPCs seeded across six previously-uncovered Tatooine commercial and civic
service rooms — Kayson's Weapon Shop, Police Station, Spaceport Speeders,
Dockside Cafe, The Cutting Edge Clinic, and Dim-U Monastery.
All must satisfy engine/hunting_rewards.is_huntable_mob():
...
Test sections:
  ...
  6. TestRoomsExist        — every room name resolves in tatooine.yaml
...
MOB_FILE = os.path.join(CW_DIR, "npcs_drop_mob_grind_tatooine_commerce_services.yaml")
ERA_FILE = os.path.join(CW_DIR, "era.yaml")
TATOOINE_PLANET_FILE = os.path.join(CW_DIR, "planets", "tatooine.yaml")
...
class TestEraManifestRef(unittest.TestCase):
    def test_file_in_era_yaml(self):
        ...
        ref_name = "npcs_drop_mob_grind_tatooine_commerce_services.yaml"
        self.assertIn(
            ref_name, npcs_refs,
