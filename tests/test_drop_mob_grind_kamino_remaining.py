# -*- coding: utf-8 -*-
"""
tests/test_drop_mob_grind_kamino_remaining.py

Verifies the Kamino Remaining Rooms hostile-mob grind batch
(data/worlds/clone_wars/npcs_drop_mob_grind_kamino_remaining.yaml).

Covers the four Kamino rooms that lacked hostile-mob coverage after
all prior batches: Tipoca City Arrivals Hall, Clone Barracks,
Aiwha Stable, Ocean Platform - Visiting Officers' Quarters.

All four NPCs must satisfy engine/hunting_rewards.is_huntable_mob():
  - ai_config.hostile = true
  - none of the special-reward exclusion markers set
...
MOB_FILE = os.path.join(CW_DIR, "npcs_drop_mob_grind_kamino_remaining.yaml")
...
class TestYamlParses(unittest.TestCase):
    ...
    def test_four_npcs(self):
        data = _load_yaml(MOB_FILE)
        self.assertGreaterEqual(len(data["npcs"]), 4, "Expected at least 4 hostile mobs")
...
class TestEraManifestRef(unittest.TestCase):
    def test_file_in_era_yaml(self):
        ...
        ref_name = "npcs_drop_mob_grind_kamino_remaining.yaml"
        self.assertIn(
            ref_name, npcs_refs,
