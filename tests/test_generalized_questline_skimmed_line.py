# -*- coding: utf-8 -*-
"""
tests/test_generalized_questline_skimmed_line.py — T3.24 generalized
quest expansion, fourth slice.

Proves the FOURTH accessible (non-t5, non-tutorial) `kind: questline`
chain — "The Skimmed Line" (kuat_ring_skimmed_line) — is shipped
correctly and walks start->graduation through the PRODUCTION dispatcher,
the same hooks the live parser calls. Like the first three slices (The
Ghost Shipment, The Crooked Wheel, The Lost Courier) it reuses the live
questline engine (active_questline slot, the existing event types, the
four reward funnels) with NO new engine code, per
quest_expansion_postlaunch_path_v1.md.

Distinct from the prior three:
  * set on Kuat (the Kuat Drive Yards orbital ring), not Nar Shaddaa
    (Ghost Shipment) / Tatooine/Mos Eisley (Crooked Wheel) / Coruscant
    lower city (Lost Courier) — the four accessible questlines now span
    four planets;
  * exercises a FOURTH distinct skill spread — SECURITY + BARGAIN (vs
    search+streetwise / investigation+gambling / persuasion+sneak) — so
    the four accessible questlines each teach distinct play;
  * a clear-the-innocent industrial-theft story shape, not a racket-bust
    or a rescue.
...
QUESTLINE_ID = "kuat_ring_skimmed_line"
ACHIEVEMENT_KEY = "skimmed_line_cleared"
GIVER_NPC = "Dav Nuro"
ANTAGONIST_NPC = "Suro Vant"
ENEMY_TEMPLATE = "kuat_skimmed_line_collector"
START_ROOM = "kuat_ring_cantina"
GIVER_ROOM_NAME = "Kuat Drive Yards - The Pressure Drop Cantina"
NPC_FILE = (PROJECT_ROOT / "data" / "worlds" / "clone_wars"
            / "npcs_drop_generalized_questline_skimmed_line.yaml")
...
class TestWalkthrough(_RealCorpusBase):
    ...
    def test_full_walkthrough_to_graduation(self):
        ...
        # Step 1: talk to Dav Nuro (the foreman)
        _run(on_talk_to_npc(db, char, GIVER_NPC))
        self.assertEqual(_qstate(char).get("step"), 2)

        # Step 2: slice the scrubbed shift log (security)
        _run(on_skill_check_passed(db, char, "security", True,
                                   difficulty=10))
        self.assertEqual(_qstate(char).get("step"), 3)

        # Step 3: bargain the warehouse fence (bargain skill check)
        _run(on_skill_check_passed(db, char, "bargain", True,
                                   difficulty=12))
        self.assertEqual(_qstate(char).get("step"), 4)

        # Step 4: defeat Suro Vant (combat_won, count 1)
        _run(on_combat_won(db, char, ENEMY_TEMPLATE, 1))
        self.assertEqual(_qstate(char).get("step"), 5)

        # Step 5: return to Dav -> graduate
        _run(on_talk_to_npc(db, char, GIVER_NPC))
    ...
    def test_skill_failure_does_not_advance(self):
        ...
        _run(on_skill_check_passed(db, char, "security", False,
                                   difficulty=10))
        self.assertEqual(_qstate(char).get("step"), 2)  # no advance

    def test_wrong_skill_does_not_advance(self):
        # Step 2 gates on security; a passing persuasion check (the Lost
        # Courier's skill) must NOT advance this questline.
        from engine.chain_events import (
            start_questline, on_talk_to_npc, on_skill_check_passed,
        )
        char = _char()
...
class TestNpcs(_RealCorpusBase):
    ...
    def test_giver_present_in_start_room(self):
        ...
        # Display name of kuat_ring_cantina.
    ...
    def test_npc_file_wired_into_era(self):
        ...
        self.assertIn(
            "npcs_drop_generalized_questline_skimmed_line.yaml", npc_refs)
