# -*- coding: utf-8 -*-
"""
tests/test_drop_mob_grind_nar_shaddaa.py

Verifies the Nar Shaddaa hostile-mob grind batch
(data/worlds/clone_wars/npcs_drop_mob_grind_nar_shaddaa.yaml).
...
MOB_FILE = os.path.join(CW_DIR, "npcs_drop_mob_grind_nar_shaddaa.yaml")
...
class TestEraClean(unittest.TestCase):
    ...
    def test_no_banned_strings(self):
        # Comments are exempt per B3 invariant — only check non-comment lines.
...
class TestEraManifestRef(unittest.TestCase):
    def test_file_in_era_yaml(self):
        ...
        ref_name = "npcs_drop_mob_grind_nar_shaddaa.yaml"
        self.assertIn(
            ref_name, npcs_refs,
