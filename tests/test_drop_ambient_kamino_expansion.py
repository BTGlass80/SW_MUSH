# -*- coding: utf-8 -*-
"""
tests/test_drop_ambient_kamino_expansion.py
Verifies the Kamino ambient population expansion NPC batch
(data/worlds/clone_wars/npcs_drop_ambient_kamino_expansion.yaml).

Covers:
  1. File exists and YAML parses cleanly
  2. All 5 expected NPCs present by name
...
YAML_PATH = os.path.join(CW_DIR, "npcs_drop_ambient_kamino_expansion.yaml")
ERA_PATH = os.path.join(CW_DIR, "era.yaml")

EXPECTED_NPCS = {
    "Kaminoan Archivist Shev Tara",
    "Republic Contracts Attaché Daro Fen",
    "Clone Trooper CT-7722 'Bracket'",
    "Kaminoan Aiwha-tender Vel Nuri",
    "Clone Trooper CT-3372 'Splash'",
}

EXPECTED_ROOMS = {
    "Kaminoan Archivist Shev Tara": "Kamino - Tipoca City Library and Research Archive",
    "Republic Contracts Attaché Daro Fen": "Kamino - Tipoca City - Visitor Dining Hall",
    "Clone Trooper CT-7722 'Bracket'": "Kamino - Clone Mess Hall",
    "Kaminoan Aiwha-tender Vel Nuri": "Kamino - Aiwha Stable",
    "Clone Trooper CT-3372 'Splash'": "Kamino - Ocean Platform - Visiting Officers' Quarters",
}
...
class TestAmbientExpansionYamlFile(unittest.TestCase):
    ...
    def test_npc_count(self):
        data = _load_yaml(YAML_PATH)
        self.assertEqual(len(data["npcs"]), 5, "Expected exactly 5 ambient Kamino expansion NPCs")
...
class TestEraManifestRegistered(unittest.TestCase):
    def test_yaml_in_era_npcs_list(self):
        ...
        self.assertIn("npcs_drop_ambient_kamino_expansion.yaml", names,
                      "npcs_drop_ambient_kamino_expansion.yaml not in era.yaml content_refs.npcs")
