"""
tests/test_mob_grind_hutt_territory.py

Static invariant tests for the Hutt Territory hostile mob grind batch
(data/worlds/clone_wars/npcs_drop_mob_grind_hutt_territory.yaml).

Covers 4 Hutt-controlled contested rooms across two planets:
...
Verifies:
  ...
  2. All 4 NPCs are present
  3. Every NPC satisfies is_huntable_mob() requirements
  4. Room names match real rooms in nar_shaddaa.yaml or tatooine.yaml
  5. All rooms are in the expected target set
  6. Each expected room has at least one NPC
  7. Weapon keys exist in data/weapons.yaml
  8. Era cleanness — no forbidden strings
  9. Required char_sheet fields present
  10. Schema version is 1
  11. Species diversity — all 4 distinct species
  12. Fallback lines present on every NPC
...
GRIND_FILE = os.path.join(
    DATA_DIR, "npcs_drop_mob_grind_hutt_territory.yaml"
)
NAR_SHADDAA_FILE = os.path.join(DATA_DIR, "planets", "nar_shaddaa.yaml")
TATOOINE_FILE = os.path.join(DATA_DIR, "planets", "tatooine.yaml")
...
@pytest.fixture(scope="module")
def all_planet_room_names():
    """Room names from both nar_shaddaa.yaml and tatooine.yaml combined."""
    names = set()
    for planet_file in [NAR_SHADDAA_FILE, TATOOINE_FILE]:
        with open(planet_file, encoding="utf-8") as f:
            data = yaml.safe_load(f)
        names.update(room["name"] for room in data.get("rooms", []))
    return names
...
def test_rooms_are_valid_planet_rooms(grind_data, all_planet_room_names):
    """Every NPC's room matches a real room in nar_shaddaa.yaml or tatooine.yaml."""
    for npc in grind_data["npcs"]:
        assert npc["room"] in all_planet_room_names, (
            f"{npc['name']}: room {npc['room']!r} not found in nar_shaddaa.yaml or tatooine.yaml"
        )


def test_rooms_are_expected_target_rooms(grind_data):
    """Every NPC is placed in one of the four target Hutt territory rooms."""
    for npc in grind_data["npcs"]:
        assert npc["room"] in EXPECTED_ROOMS, (
            f"{npc['name']}: room {npc['room']!r} is not a target Hutt territory room"
        )


def test_each_expected_room_covered(grind_data):
    """All four expected rooms have at least one NPC."""
...
def test_era_cleanness(grind_data):
    ...
    for term in ERA_FORBIDDEN:
        assert term not in raw, (
            f"Era violation: {term!r} found in Hutt territory mob grind data"
        )
...
def test_registered_in_era_yaml(era_data):
    ...
    assert "hutt_territory" in raw, (
        "npcs_drop_mob_grind_hutt_territory.yaml not registered in era.yaml"
    )


def test_species_diversity(grind_data):
    """All 4 NPCs are different species — the batch covers distinct Hutt enforcer types."""
...
def test_all_hutt_faction(grind_data):
    """Every NPC has faction: hutt_cartel (Hutt territory batch)."""
    for npc in grind_data["npcs"]:
        assert npc["ai_config"].get("faction") == "hutt_cartel", (
            f"{npc['name']}: expected faction hutt_cartel, "
            f"got {npc['ai_config'].get('faction')!r}"
        )
