"""
tests/test_mob_grind_coruscant_civic_government.py

Static invariant tests for the Coruscant Civic Core & Government Hall
hostile mob grind batch
(data/worlds/clone_wars/npcs_drop_mob_grind_coruscant_civic_government.yaml).

Covers 6 rooms across 2 Coruscant zones:
  senate_district (5):
    Coruscant - Galactic Senate Building - Exterior Plaza
    Coruscant - Galactic Senate Building - Public Lobby
    Coruscant - Chancellor's Avenue
    Coruscant - Grand Reception Hall
    Coruscant - Legislative Borough
  entertainment_district (1):
...
Verifies:
  ...
  5. Room names match real rooms in coruscant.yaml
...
GRIND_FILE = os.path.join(DATA_DIR, "npcs_drop_mob_grind_coruscant_civic_government.yaml")
...
def test_rooms_are_valid_coruscant_rooms(grind_data, coruscant_room_names):
    """Every NPC's room matches a real room in coruscant.yaml."""
    for npc in grind_data["npcs"]:
...
def test_rooms_are_expected_target_rooms(grind_data):
    ...
    for npc in grind_data["npcs"]:
        assert npc["room"] in EXPECTED_ROOMS, (
            f"{npc['name']}: room {npc['room']!r} is not a target civic-government room"
        )
...
def test_era_cleanness(grind_data):
    ...
    for term in ERA_FORBIDDEN:
        assert term not in raw, (
            f"Era violation: {term!r} found in Coruscant civic-government mob grind data"
        )
...
def test_registered_in_era_yaml(era_data):
    ...
    assert "civic_government" in raw, (
        "npcs_drop_mob_grind_coruscant_civic_government.yaml not registered in era.yaml"
    )
