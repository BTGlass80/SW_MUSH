# -*- coding: utf-8 -*-
"""
tests/test_drop_mob_grind_tatooine_jundland.py

Verifies the Tatooine Jundland Wastes interior hostile-mob grind expansion
(data/worlds/clone_wars/npcs_drop_mob_grind_tatooine_jundland.yaml).

Two NPCs fill the two Jundland rooms previously without mob coverage:
  - Jundland Wastes - Krayt Dragon Graveyard (Young Canyon Krayt)
  - Jundland Wastes - Hidden Cave            (Outlaw Cache Keeper)

Both must satisfy engine/hunting_rewards.is_huntable_mob().
...
MOB_FILE = os.path.join(CW_DIR, "npcs_drop_mob_grind_tatooine_jundland.yaml")
...
class TestYamlParses(unittest.TestCase):
    ...
    def test_two_npcs(self):
        data = _load_yaml(MOB_FILE)
        self.assertGreaterEqual(len(data["npcs"]), 2, "Expected at least 2 hostile mobs")
...
class TestEraManifestRef(unittest.TestCase):
    def test_file_in_era_yaml(self):
        ...
        ref_name = "npcs_drop_mob_grind_tatooine_jundland.yaml"
        self.assertIn(
            ref_name, npcs_refs,
