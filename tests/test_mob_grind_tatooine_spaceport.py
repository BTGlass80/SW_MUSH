"""
tests/test_mob_grind_tatooine_spaceport.py

Static invariant tests for the Tatooine Spaceport Docking Bays hostile mob grind
batch (data/worlds/clone_wars/npcs_drop_mob_grind_tatooine_spaceport.yaml).

Covers 6 previously-uncovered Tatooine spaceport rooms (tatooine_spaceport zone):
  Docking Bay 86
  Docking Bay 87
  Docking Bay 92
...
Verifies:
  ...
  5. Room names match real rooms in tatooine.yaml
  ...
  9. Required char_sheet fields present on every NPC
  10. Era cleanness — no forbidden strings
  11. Species diversity — all 6 distinct species
  12. Fallback lines present (>=2) on every NPC
...
GRIND_FILE = os.path.join(DATA_DIR, "npcs_drop_mob_grind_tatooine_spaceport.yaml")
...
@pytest.fixture(scope="module")
def tatooine_room_names():
    with open(TATOOINE_FILE, encoding="utf-8") as f:
...
def weapon_keys():
    ...
    for line in content.splitlines():
        ...
        if stripped.startswith("key:"):
            ...
            if key:
                keys.add(key)
    # Also collect top-level YAML keys (weapon definitions)
    for line in content.splitlines():
...
def test_rooms_are_valid_tatooine_rooms(grind_data, tatooine_room_names):
    """Every NPC's room matches a real room in tatooine.yaml."""
    for npc in grind_data["npcs"]:
        assert npc["room"] in tatooine_room_names, (
            f"{npc['name']}: room {npc['room']!r} not found in tatooine.yaml"
        )


def test_rooms_are_expected_target_rooms(grind_data):
    """Every NPC is placed in one of the six target spaceport rooms."""
    for npc in grind_data["npcs"]:
        assert npc["room"] in EXPECTED_ROOMS, (
            f"{npc['name']}: room {npc['room']!r} is not a target spaceport room"
        )
...
def test_era_cleanness(grind_data):
    ...
    for term in ERA_FORBIDDEN:
        assert term not in raw, (
            f"Era violation: {term!r} found in Tatooine spaceport mob grind data"
        )
...
def test_registered_in_era_yaml(era_data):
    ...
    assert "mob_grind_tatooine_spaceport" in raw, (
        "npcs_drop_mob_grind_tatooine_spaceport.yaml not registered in era.yaml"
    )


def test_species_diversity(grind_data):
    """All 6 NPCs are different species — the batch covers distinct character types."""
