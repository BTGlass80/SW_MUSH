# -*- coding: utf-8 -*-
"""
tests/test_generalized_questline_wreckers_light.py — T3.24 generalized
quest expansion, twentieth slice.

Proves the TWENTIETH accessible (non-t5, non-tutorial) `kind: questline`
chain — "The Wreckers' Light" (nar_shaddaa_wreckers_light) — is shipped correctly
and walks start->graduation through the PRODUCTION dispatcher, the same hooks the
live parser calls. Like the first nineteen slices (The Ghost Shipment, The Crooked
...
Wasting Ward, The Rigged Issue, The Salted Lane, The Short Weight) it reuses the
live questline engine (active_questline slot, the existing event types, the four
reward funnels) with NO new engine code, per quest_expansion_postlaunch_path_v1.md.

Firsts for the accessible-questline arc:
  * a TWENTIETH distinct skill spread — COMMUNICATIONS (Mechanical) + PLANETARY
    SYSTEMS (Knowledge) — neither of which any prior accessible questline uses (vs
    ...
    gunnery / powersuit-operation+lifting). The FIRST accessible questline to
    reward the SIGNALS-AND-LANES INTEL build — a comm board and a chart table. It
    is mechanically DISTINCT from the only prior arc to touch a comm/sensor feed,
    The Sabotaged Run (sensors + repulsorlift operation, a ground speeder racer
    reading doctored telemetry — SENSORS, not the comm set), and from the only
    prior arc to plot the void, The Condemned Hull (space transport repair +
    astrogation, a starship engineer-navigator plotting a HYPERSPACE JUMP and
    servicing a sealed hull — not a system's sublight approach lanes and hazards);
  * the SECOND questline set on NAR SHADDAA and the FIRST on the CORELLIAN-SECTOR /
    PROMENADE DOCKS face (old_corellian_quarter / the_grid /
    promenade_corporate_tower / vertical_bazaar — the old spacer enclave and the
    Promenade trade-tower district), every room of which is FRESH to the entire
    chain corpus — reusing NONE of The Wasting Ward's refugee-and-undercity rooms.
    The Hutt-front dock authority the upper-dock and Grid room text name stays
    distant background — and is the pragmatic local power a freelance wreck-luring
    outfit poaching the moon's own approach lanes runs afoul of — so the war stays
    offstage the way every prior accessible arc keeps the Republic/CIS conflict
    offstage;
  * a foil who carries the proven blaster_pistol of the ranged foils, squarely in
    ...
    Lane's and The Short Weight's foils).

The story shape is new too — breaking a FALSE-BEACON / WRECK-LURING (salvage)
racket; no prior accessible arc touches one. The Condemned Hull condemns a SOUND
ship via certification fraud to seize it cheap; this DESTROYS ships outright with a
faked beacon and harvests the wrecks. The Long Haul strands rigs on the road to tow
them off under a salvage lien, crews and cargo intact; this lures ships to their
deaths and strips the wrecks. The Salted Lane bends a freighter out to a partnered
hijack crew that STRIPS the ship, crew and hull walking away; this leaves no ship
and no crew to walk away. A freelance salvage outfit plants spoofed distress beacons
on the outer Corellian-sector approach that drag ships off the corridor into the
Saljenn Drift, logs the dead ships as "derelicts," files salvage claims, and strips
the hulls while the dock authority writes the losses up as bad piloting — so work
the broker's comm-feed grid to capture the spoofed beacon and triangulate the
transmitter that fakes it (communications), lay the outfit's filed claims over the
true approach lanes to prove every wreck sits off the corridor on the beacon's
drag-bearing in the Drift (planetary systems), stand off the outfit's claims-agent
who comes for the proof (combat_won), and get the beacon trace and the claim overlay
to the dock authority's claims board. It carries a real combat climax (step 4), with
a single placed antagonist NPC and a chain_enemy_template.
...
achievement, real Nar Shaddaa room slugs, the giver + foil NPCs, the combat-climax
structure, the all-Nar-Shaddaa-rooms + every-room-fresh-to-corpus claim, the
twentieth-distinct-spread + comms(Mechanical)/lanes(Knowledge) claim + distinctness
from the prior sensor/comm arc and the prior void-plotting arc, and that the authored
spread skills resolve to a trained character's real pool at `chain attempt` (not the
raw attribute).
...
QUESTLINE_ID = "nar_shaddaa_wreckers_light"
ACHIEVEMENT_KEY = "wreckers_light_cleared"
GIVER_NPC = "Corwin Hale"
ANTAGONIST_NPC = "Hask Vorran"
ENEMY_TEMPLATE = "wreckers_light_enforcer"
START_ROOM = "old_corellian_quarter"
GIVER_ROOM_NAME = "Nar Shaddaa - Old Corellian Quarter"
ANTAGONIST_ROOM_NAME = "Nar Shaddaa - Vertical Bazaar"
NPC_FILE = (PROJECT_ROOT / "data" / "worlds" / "clone_wars"
            / "npcs_drop_generalized_questline_wreckers_light.yaml")

# The twentieth skill spread, in step order (steps 2/3 are skill_check_passed).
EXPECTED_SKILLS = ["communications", "planetary systems"]

# The skill spreads of the prior NINETEEN accessible questlines (each non-combat
# skill that gates a skill_check_passed step). The twentieth spread must share
# NO skill with any of them — the "twentieth DISTINCT spread" claim.
PRIOR_SPREAD_SKILLS = {
...
# The only prior arc to work a comm/sensor feed — a ground speeder racer reading
# doctored telemetry off the SENSOR board, not a comm board. The twentieth's
# Mechanical skill (communications) shares no spread skill with it.
SABOTAGED_RUN_SPREAD = {"sensors", "repulsorlift operation"}
# The only prior arc to plot the void — a starship engineer-navigator plotting a
# HYPERSPACE JUMP (astrogation) and servicing a sealed hull, not a system's
# sublight approach lanes and hazards. The twentieth's Knowledge skill (planetary
# systems) shares no spread skill with it.
...
class TestWalkthrough(_RealCorpusBase):
    ...
    def test_full_walkthrough_to_graduation(self):
        ...
        # Step 1: talk to Corwin Hale (the captain)
        _run(on_talk_to_npc(db, char, GIVER_NPC))
        self.assertEqual(_qstate(char).get("step"), 2)

        # Step 2: capture + triangulate the spoofed beacon (communications)
        _run(on_skill_check_passed(db, char, "communications", True,
                                   difficulty=11))
        self.assertEqual(_qstate(char).get("step"), 3)

        # Step 3: lay claims over the true approach lanes (planetary systems)
        _run(on_skill_check_passed(db, char, "planetary systems", True,
                                   difficulty=13))
        self.assertEqual(_qstate(char).get("step"), 4)

        # Step 4: stop Hask Vorran in the vertical bazaar (combat_won, count 1)
        _run(on_combat_won(db, char, ENEMY_TEMPLATE, 1))
        self.assertEqual(_qstate(char).get("step"), 5)

        # Step 5: return to Corwin Hale -> graduate
        _run(on_talk_to_npc(db, char, GIVER_NPC))
    ...
    def test_skill_failure_does_not_advance(self):
        ...
        _run(on_skill_check_passed(db, char, "communications", False,
                                   difficulty=11))
        self.assertEqual(_qstate(char).get("step"), 2)  # no advance

    def test_wrong_skill_does_not_advance(self):
        # Step 2 gates on communications; a passing planetary systems check (this
...
class TestReachabilityBits(_RealCorpusBase):
    ...
    def test_skill_spread_is_communications_then_planetary_systems(self):
        # The twentieth distinct spread: the two skill_check_passed steps gate
        # on communications then planetary systems (no prior accessible
    ...
    def test_spread_is_distinct_from_all_prior_arcs(self):
        # The "twentieth DISTINCT spread" claim: neither spread skill is used
        # by any of the prior nineteen accessible questlines.
        self.assertFalse(
    ...
    def test_spread_is_signals_and_lanes(self):
        # The defining first: the spread is the SIGNALS-AND-LANES INTEL build —
        # communications (Mechanical, the comm board) + planetary systems
        # (Knowledge, the chart table). Grounded against the live skills.yaml.
        ...
        self.assertIn("communications", mechanical,
                      "'communications' is not a Mechanical skill — the "
                      "comm-board claim is false")
        self.assertIn("planetary systems", knowledge,
                      "'planetary systems' is not a Knowledge skill — the "
                      "chart-table claim is false")

    def test_distinct_from_prior_sensor_and_void_arcs(self):
        # The differentiation from the only prior arc to work a comm/sensor feed
        # (The Sabotaged Run, a ground speeder racer reading SENSORS) and the
        # only prior arc to plot the void (The Condemned Hull, an
        # astrogation-and-repair engineer-navigator). This works a COMM board and
        # a chart table, and shares NO spread skill with either.
        self.assertFalse(
            set(EXPECTED_SKILLS) & SABOTAGED_RUN_SPREAD,
            "The Wreckers' Light shares a spread skill with The Sabotaged Run — "
            "the 'comm board, not the sensor board' claim is false")
        self.assertFalse(
            set(EXPECTED_SKILLS) & CONDEMNED_HULL_SPREAD,
            "The Wreckers' Light shares a spread skill with The Condemned Hull — "
            "the 'approach lanes, not a hyperspace jump' claim is false")
    ...
    def test_spread_skills_resolve_to_trained_pools(self):
        ...
        trained = {
            ...
            "skills": json.dumps({"communications": "5D"}),
        }
        untrained = {
            "attributes": json.dumps({"mechanical": "3D"}),
            "skills": json.dumps({}),
        }
        trained_pool = _get_skill_pool(trained, "communications", reg)
        raw_pool = _get_skill_pool(untrained, "communications", reg)
        # A char who trained Communications must roll a STRICTLY larger pool than
        # a char rolling raw Mechanical — proving the authored skill resolves to
        # the trained skill rather than the bare attribute.
        self.assertGreater(
            trained_pool, raw_pool,
            "authored spread skill 'communications' must roll the trained "
            "pool, not raw Mechanical")

    def test_all_step_rooms_are_real_nar_shaddaa_rooms(self):
        # The "second questline set on Nar Shaddaa / Corellian-sector docks face"
        # claim: every step room (and the drop room) is a real loaded Nar Shaddaa
        # room (planets/nar_shaddaa.yaml).
        nar = _nar_shaddaa_room_slugs()
        ql = self._questline()
        for step in ql.steps:
            self.assertIn(step.location, nar,
                          f"step {step.step} location {step.location!r} is not "
                          f"a Nar Shaddaa room — the 'second Nar Shaddaa arc' "
                          f"claim is false")
    ...
    def test_every_room_is_fresh_to_the_corpus(self):
        ...
        # ANY other chain in the corpus (in particular none of The Wasting Ward's
        # refugee-and-undercity rooms).
        ql = self._questline()
        ...
        self.assertFalse(
            overlap,
            f"The Wreckers' Light reuses rooms already in the chain corpus "
...
class TestNpcs(_RealCorpusBase):
    ...
    def test_giver_present_in_start_room(self):
        ...
        # Display name of old_corellian_quarter.
    ...
    def test_antagonist_carries_proven_ranged_weapon(self):
        # Back in the proven beatable band: Hask Vorran carries blaster_pistol
    ...
    def test_exactly_two_placed_npcs(self):
        ...
        # salvage outfit's boss, his offworld fence, the spoofed beacons and the
        # transmitter that fakes them, the lost ships and dead crews, the
        # Saljenn Drift, the information-broker and his Gamorrean sentinels, the
        # Hutt-front dock authority and its claims board, and the scared
        # independent spacers are narrated-only.
        self.assertEqual(len(self.npcs), 2,
                         "The Wreckers' Light should place exactly two NPCs "
    ...
    def test_npc_file_wired_into_era(self):
        ...
        self.assertIn(
            "npcs_drop_generalized_questline_wreckers_light.yaml", npc_refs)
