# -*- coding: utf-8 -*-
"""
tests/test_drop_mob_grind_tatooine_mos_eisley_expansion.py

Verifies the Tatooine Mos Eisley expansion hostile-mob grind batch
(data/worlds/clone_wars/npcs_drop_mob_grind_tatooine_mos_eisley_expansion.yaml).
...
MOB_FILE = os.path.join(CW_DIR, "npcs_drop_mob_grind_tatooine_mos_eisley_expansion.yaml")
...
class TestYamlParses(unittest.TestCase):
    ...
    def test_four_npcs(self):
        data = _load_yaml(MOB_FILE)
        self.assertEqual(len(data["npcs"]), 4, "Expected exactly 4 hostile mobs")
...
class TestEraManifestRef(unittest.TestCase):
    def test_file_in_era_yaml(self):
        ...
        ref_name = "npcs_drop_mob_grind_tatooine_mos_eisley_expansion.yaml"
        self.assertIn(
            ref_name, npcs_refs,
