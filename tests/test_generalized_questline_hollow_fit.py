# -*- coding: utf-8 -*-
"""
tests/test_generalized_questline_hollow_fit.py — T3.24 generalized
quest expansion, thirty-second slice.

Proves the THIRTY-SECOND accessible (non-t5, non-tutorial) `kind: questline`
chain — "The Hollow Fit" (mos_eisley_hollow_fit) — is shipped correctly and
walks start->graduation through the PRODUCTION dispatcher, the same hooks the
live parser calls. Like the first thirty-one slices it reuses the live
...
Firsts for the accessible-questline arc:
  * a THIRTY-SECOND distinct skill spread — STARSHIP WEAPON REPAIR (Technical) +
    STARSHIP SHIELDS (Mechanical) — neither of which any prior accessible
    questline uses. It is the FIRST accessible questline to reward the SHIP
    DEFENSIVE-SYSTEMS TECHNICIAN build, the FIRST to use STARSHIP WEAPON REPAIR,
    and the FIRST to use STARSHIP SHIELDS. (This IS a cross-attribute spread —
    starship weapon repair is Technical, starship shields is Mechanical — but it
    is NOT the first Mechanical+Technical cross: The Condemned Hull, The Long
    Haul, and The Rolled Log crossed those before it. What is genuinely first is
    the starship-shields and starship-weapon-repair skills and the
    defensive-systems / prove-the-fit-is-hollow pairing: one hand strips a
    "fitted" cannon to show it can't fire, the other runs a "full-rated" shield
    to show it folds. MEMORY-CORRECTION: starship shields and starship weapon
    repair are SMALL-SHIP skills — the "starship" class, sibling to space
    transports / starfighter — NOT the big-ship capital-ship pool, so the arc
    works a light freighter in a civilian docking pit and sidesteps the
    big-ship-room problem.);
  * set on TATOOINE, spanning the tatooine_cantina and tatooine_spaceport zones
    (chalmuans_cantina_entrance / chalmuans_cantina_back_hallway /
    docking_bay_94_entrance / spaceport_hotel), every room of which is FRESH to
    the entire chain corpus. The Crooked Wheel worked Chalmun's main bar and The
    Sabotaged Run / The Short Weight / The Rolled Log worked the spaceport, so
    this is honestly the ELEVENTH Tatooine arc, NEITHER a fresh face NOR a
    single-zone cluster: it is a CROSS-ZONE assembly of four rooms, precedented
    by The Empty Proof's tatooine_outskirts + tatooine_jundland cluster. Only the
    step-4 combat room must be combat-capable (spaceport_hotel is contested); the
    giver/return room and the two skill rooms host no fight. The
    Senate/Jedi/Republic war effort have no reach at a fringe ship-outfitter and
    never appear; the racket is run afoul of by the traders' own berth registry,
  ...
  * a foil who carries the proven blaster_pistol of the ranged foils, squarely
    ...
    Salted Lane's ... The Cold Charge's foils), pointedly NOT the melee line of
    The Condemned Hull's out-of-band Houk foil.

The story shape is new too — breaking a HOLLOW-FIT / DEFENSIVE-SYSTEMS fraud,
the first accessible arc whose racket is selling a ship a hollow shell of the
defenses it paid for and fencing its real teeth to the predators. A ship-defense
outfitter paid by the fringe haulers to fit and certify their light freighters'
deflector shields and hull-mount cannons instead installs gutted units, certifies
them full-rated on forged fit-sheets, and fences the pulled real cores and the
haulers' run schedules to the raiders who prey on those lanes. So strip a
"fitted" cannon mount off the shop's re-fit bench and read it dead (starship
weapon repair), run a "full-rated" freighter's deflector fit up until it folds
far below its sheet (starship shields), stand off the shop's enforcer at the
Spaceport Hotel (combat_won), and put the hollow fit on the traders' berth
registry. Pointedly DISTINCT from the prior arcs it might be mistaken for: The
Empty Proof SKIPS a proof on un-tested goods; The Rigged Issue SUBSTITUTES
counterfeits; The Cut Coil DAMAGES a sound speeder to bill a repair; The Rolled
Log rolls back a CRAFT's history; The Cold Charge fakes the DESTRUCTION of
ordnance — here the FIT of live defenses is hollowed and certified full. It
...
QUESTLINE_ID = "mos_eisley_hollow_fit"
ACHIEVEMENT_KEY = "hollow_fit_cleared"
GIVER_NPC = "Nyla Sabo"
ANTAGONIST_NPC = "Draxo Venn"
ENEMY_TEMPLATE = "hollow_fit_enforcer"
START_ROOM = "chalmuans_cantina_entrance"
GIVER_ROOM_NAME = "Chalmun's Cantina - Entrance"
ANTAGONIST_ROOM_NAME = "Spaceport Hotel"
COMBAT_ROOM_SLUG = "spaceport_hotel"
# A CROSS-ZONE cluster: the four rooms span two Tatooine zones.
CLUSTER_ZONES = {"tatooine_cantina", "tatooine_spaceport"}
NPC_FILE = (PROJECT_ROOT / "data" / "worlds" / "clone_wars"
            / "npcs_drop_generalized_questline_hollow_fit.yaml")

# The thirty-second skill spread, in step order (steps 2/3 are skill_check_passed).
EXPECTED_SKILLS = ["starship weapon repair", "starship shields"]

# The skill spreads of the prior THIRTY-ONE accessible questlines (each non-combat
# skill that gates a skill_check_passed step). The thirty-second spread must
# share NO skill with any of them — the "thirty-second DISTINCT spread" claim.
PRIOR_SPREAD_SKILLS = {
...
class TestWalkthrough(_RealCorpusBase):
    ...
    def test_full_walkthrough_to_graduation(self):
        ...
        # Step 1: talk to Nyla Sabo (the ship-tech)
        _run(on_talk_to_npc(db, char, GIVER_NPC))
        self.assertEqual(_qstate(char).get("step"), 2)

        # Step 2: strip a hollow hull-cannon mount (starship weapon repair)
        _run(on_skill_check_passed(db, char, "starship weapon repair", True,
                                   difficulty=11))
        self.assertEqual(_qstate(char).get("step"), 3)

        # Step 3: run a "full-rated" deflector fit to a fold (starship shields)
        _run(on_skill_check_passed(db, char, "starship shields", True,
                                   difficulty=13))
        self.assertEqual(_qstate(char).get("step"), 4)

        # Step 4: stop Draxo Venn at the Spaceport Hotel (combat_won, count 1)
        _run(on_combat_won(db, char, ENEMY_TEMPLATE, 1))
        self.assertEqual(_qstate(char).get("step"), 5)

        # Step 5: return to Nyla Sabo -> graduate
        _run(on_talk_to_npc(db, char, GIVER_NPC))
    ...
    def test_skill_failure_does_not_advance(self):
        ...
        _run(on_skill_check_passed(db, char, "starship weapon repair", False,
                                   difficulty=11))
        self.assertEqual(_qstate(char).get("step"), 2)  # no advance

    def test_wrong_skill_does_not_advance(self):
        # Step 2 gates on starship weapon repair; a passing starship-shields
...
class TestReachabilityBits(_RealCorpusBase):
    ...
    def test_skill_spread_is_weapon_repair_then_shields(self):
        # The thirty-second distinct spread: the two skill_check_passed steps
        # gate on starship weapon repair then starship shields (no prior
    ...
    def test_spread_is_distinct_from_all_prior_arcs(self):
        # The "thirty-second DISTINCT spread" claim: neither spread skill is used
        # by any of the prior thirty-one accessible questlines.
        self.assertFalse(
    ...
    def test_spread_is_a_mechanical_technical_cross(self):
        # The defining build: the SHIP DEFENSIVE-SYSTEMS TECHNICIAN — starship
        # weapon repair (strip a "fitted" cannon mount to read its dead firing
        # circuit) + starship shields (run a "full-rated" deflector fit up to
        # read it fold). Grounded against the live skills.yaml: starship weapon
        # repair is TECHNICAL and starship shields is MECHANICAL — a genuine
        # cross-attribute spread (honestly NOT the first Mechanical+Technical
        # cross; that claim is not made). Each skill lives under its expected
        # attribute and NONE of the other five.
        ...
        self.assertIn("starship weapon repair", pool("technical"))
        self.assertIn("starship shields", pool("mechanical"))
        # starship weapon repair lives ONLY under technical (of the six attrs).
        for attr in ("strength", "knowledge", "perception", "mechanical",
                     "dexterity"):
            self.assertNotIn("starship weapon repair", pool(attr))
        # starship shields lives ONLY under mechanical.
        for attr in ("strength", "knowledge", "perception", "technical",
                     "dexterity"):
            self.assertNotIn("starship shields", pool(attr))

    def test_no_prior_arc_used_the_spread_skills(self):
        # The "first starship-weapon-repair / first starship-shields arc" claim:
        # no prior accessible arc used either skill. (The prior small-ship arcs,
        # The Rolled Log and The Salted Lane, used starfighter piloting/repair
        # and space transports/starship gunnery — never starship weapon repair
        # or starship shields.)
        self.assertNotIn("starship weapon repair", PRIOR_SPREAD_SKILLS,
                         "a prior arc already used starship weapon repair — the "
                         "'first starship-weapon-repair arc' claim is false")
        self.assertNotIn("starship shields", PRIOR_SPREAD_SKILLS,
                         "a prior arc already used starship shields — the 'first "
                         "starship-shields arc' claim is false")
    ...
    def test_spread_skills_resolve_to_trained_pools(self):
        ...
        trained = {
            ...
            "skills": json.dumps({"starship shields": "5D"}),
        }
        untrained = {
            "attributes": json.dumps({"mechanical": "3D"}),
            "skills": json.dumps({}),
        }
        trained_pool = _get_skill_pool(trained, "starship shields", reg)
        raw_pool = _get_skill_pool(untrained, "starship shields", reg)
        # A char who trained Starship Shields must roll a STRICTLY larger pool
        ...
        self.assertGreater(
            ...
            "authored spread skill 'starship shields' must roll the trained "
    ...
    def test_all_step_rooms_in_the_cross_zone_cluster(self):
        ...
        # real loaded room in the tatooine_cantina OR tatooine_spaceport zone,
        ...
        for step in ql.steps:
            self.assertIn(step.location, cluster,
                          f"step {step.step} location {step.location!r} is not "
                          f"in the tatooine_cantina/tatooine_spaceport cluster")
    ...
    def test_combat_room_is_not_secured(self):
        ...
        # Spaceport Hotel is contested. (The giver/return and skill rooms host
        # no fight, so their security is immaterial.)
        ql = self._questline()
    ...
    def test_every_room_is_fresh_to_the_corpus(self):
        ...
        # by ANY other chain in the corpus (in particular none of The Crooked
        # Wheel's cantina rooms or the spaceport arcs' docking-bay rooms).
        ql = self._questline()
        ...
        self.assertFalse(
            overlap,
            f"The Hollow Fit reuses rooms already in the chain corpus "
...
class TestNpcs(_RealCorpusBase):
    ...
    def test_giver_present_in_start_room(self):
        ...
        # Display name of chalmuans_cantina_entrance.
    ...
    def test_antagonist_carries_proven_ranged_weapon(self):
        # Back in the proven beatable band: Draxo Venn carries blaster_pistol
    ...
    def test_giver_spread_embodies_the_quest_skills(self):
        # The giver's sheet is skewed to the shields-and-gun-mount of a
        # defensive-systems fit-hand — she embodies the two skills the quest
        # sends a hand to use.
        giver = self.by_name[GIVER_NPC]
    ...
    def test_exactly_two_placed_npcs(self):
        ...
        # proprietor (Bevin Torr), the fit-sheet clerk, the fringe raiders and
        # fences, the trader and supply pools, the gutted generators and cannon
        # mounts, the haulers boarded on the lanes, and the berth registry are
        # narrated-only.
        self.assertEqual(len(self.npcs), 2,
                         "The Hollow Fit should place exactly two NPCs "
    ...
    def test_npc_file_wired_into_era(self):
        ...
        self.assertIn(
            "npcs_drop_generalized_questline_hollow_fit.yaml", npc_refs)
