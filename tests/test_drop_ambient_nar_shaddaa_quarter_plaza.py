# -*- coding: utf-8 -*-
"""
tests/test_drop_ambient_nar_shaddaa_quarter_plaza.py

Static-invariant tests for the Nar Shaddaa Old Corellian Quarter &
Warrens Collapsed Plaza ambient NPC batch
(data/worlds/clone_wars/npcs_drop_ambient_nar_shaddaa_quarter_plaza.yaml).

Four non-hostile flavor NPCs across two previously-uncovered rooms:
  Nar Shaddaa - Old Corellian Quarter    x2  (Maret Voss / Human; Dex Keel / Human)
  Nar Shaddaa - The Warrens - Collapsed Plaza  x2  (Syek / Evocii; Renn Vass / Human)
...
Verifies:
  ...
  7. Room references resolve to real Nar Shaddaa rooms
...
YAML_PATH = os.path.join(CW_DIR, "npcs_drop_ambient_nar_shaddaa_quarter_plaza.yaml")
ERA_PATH = os.path.join(CW_DIR, "era.yaml")
NAR_SHADDAA_PATH = os.path.join(CW_DIR, "planets", "nar_shaddaa.yaml")
...
def _nar_shaddaa_room_names():
    data = _load_yaml(NAR_SHADDAA_PATH)
...
class TestRoomReferences(unittest.TestCase):
    def setUp(self):
        data = _load_yaml(YAML_PATH)
        self.npcs = data["npcs"]
        self.nar_shaddaa_rooms = _nar_shaddaa_room_names()

    def test_rooms_resolve_in_nar_shaddaa(self):
        for npc in self.npcs:
            room = npc.get("room", "")
            self.assertIn(room, self.nar_shaddaa_rooms,
                          f"{npc['name']}: room '{room}' not found in nar_shaddaa.yaml")
...
class TestEraRegistration(unittest.TestCase):
    def test_yaml_in_era_npcs_list(self):
        ...
        self.assertIn(
            "npcs_drop_ambient_nar_shaddaa_quarter_plaza.yaml",
            names,
            "npcs_drop_ambient_nar_shaddaa_quarter_plaza.yaml not in era.yaml content_refs.npcs",
        )
