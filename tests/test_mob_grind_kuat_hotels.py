"""
tests/test_mob_grind_kuat_hotels.py

Static invariant tests for the Kuat Lodging and Embassy Quarter
hostile mob grind batch
(data/worlds/clone_wars/npcs_drop_mob_grind_kuat_hotels.yaml).

Covers 3 rooms completing Kuat planet mob-grind saturation:
...
Verifies:
  ...
  5. Room names match real rooms in kuat.yaml
...
GRIND_FILE = os.path.join(DATA_DIR, "npcs_drop_mob_grind_kuat_hotels.yaml")
KUAT_FILE = os.path.join(DATA_DIR, "planets", "kuat.yaml")
...
@pytest.fixture(scope="module")
def kuat_room_names():
    with open(KUAT_FILE, encoding="utf-8") as f:
...
def test_rooms_are_valid_kuat_rooms(grind_data, kuat_room_names):
    """Every NPC's room matches a real room in kuat.yaml."""
    for npc in grind_data["npcs"]:
        assert npc["room"] in kuat_room_names, (
            f"{npc['name']}: room {npc['room']!r} not found in kuat.yaml"
        )


def test_rooms_are_expected_target_rooms(grind_data):
    """Every NPC is placed in one of the three target rooms."""
    for npc in grind_data["npcs"]:
        assert npc["room"] in EXPECTED_ROOMS, (
            f"{npc['name']}: room {npc['room']!r} is not a target hotel/embassy room"
        )


def test_each_expected_room_covered(grind_data):
    """All three expected rooms have at least one NPC."""
...
def test_era_cleanness(grind_data):
    ...
    for term in ERA_FORBIDDEN:
        assert term not in raw, (
            f"Era violation: {term!r} found in Kuat hotels mob grind data"
        )
...
def test_registered_in_era_yaml(era_data):
    ...
    assert "kuat_hotels" in raw, (
        "npcs_drop_mob_grind_kuat_hotels.yaml not registered in era.yaml"
    )
