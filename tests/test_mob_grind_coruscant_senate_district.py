"""
tests/test_mob_grind_coruscant_senate_district.py

Static invariant tests for the Coruscant Senate District & Upper-City
Expansion hostile mob grind batch
(data/worlds/clone_wars/npcs_drop_mob_grind_coruscant_senate_district.yaml).

Covers 6 rooms across 3 Coruscant zones:
  senate_district (3):
    Coruscant - Westport Spaceport
    Coruscant - Glitannai Esplanade
    Coruscant - Monument Plaza
  monumental_district (2):
    Coruscant - Calocour Heights - Marketing Row
    Coruscant - Calocour Heights - Overlook Terrace
  commercial_district (1):
    Coruscant Works - Engineering Corps Habitation Block
...
GRIND_FILE = os.path.join(DATA_DIR, "npcs_drop_mob_grind_coruscant_senate_district.yaml")
...
def test_rooms_are_expected_target_rooms(grind_data):
    """Every NPC is placed in one of the six target senate-district rooms."""
    for npc in grind_data["npcs"]:
        assert npc["room"] in EXPECTED_ROOMS, (
            f"{npc['name']}: room {npc['room']!r} is not a target senate-district room"
        )
...
def test_era_cleanness(grind_data):
    ...
    for term in ERA_FORBIDDEN:
        assert term not in raw, (
            f"Era violation: {term!r} found in Coruscant senate-district mob grind data"
        )
...
def test_registered_in_era_yaml(era_data):
    ...
    assert "senate_district" in raw, (
        "npcs_drop_mob_grind_coruscant_senate_district.yaml not registered in era.yaml"
    )
