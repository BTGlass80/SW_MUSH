# -*- coding: utf-8 -*-
"""
tests/test_generalized_questline_ghost_shipment.py — T3.24 generalized
quest expansion, first slice.

Proves the FIRST accessible (non-t5, non-tutorial) `kind: questline`
chain — "The Ghost Shipment" (nar_freight_ghost_shipment) — is shipped
correctly and walks start->graduation through the PRODUCTION dispatcher,
the same hooks the live parser calls. It reuses the live questline engine
...
Distinct from the t5 master-trainer questlines (test_t5_questline_content):
this questline is ACCESSIBLE — prerequisites: chargen_complete, no faction
rep gate — so a fresh post-chargen character can take it. The test pins
that accessibility, the modest reward band (below the tuned rep ceiling),
the registered+linked achievement, real room slugs, the giver NPC presence,
and the combat foil's chain_enemy_template tag.
...
QUESTLINE_ID = "nar_freight_ghost_shipment"
ACHIEVEMENT_KEY = "ghost_shipment_solved"
GIVER_NPC = "Yelza Korrin"
ANTAGONIST_NPC = "Brakkis Vone"
ENEMY_TEMPLATE = "nar_ghost_shipment_pirate"
START_ROOM = "burning_deck_cantina"
NPC_FILE = (PROJECT_ROOT / "data" / "worlds" / "clone_wars"
            / "npcs_drop_generalized_questline_ghost_shipment.yaml")
...
def _char(attrs: dict = None) -> dict:
    ...
    return {
        "id": 21, "name": "Freelancer PC", "room_id": 100,
...
class TestWalkthrough(_RealCorpusBase):
    ...
    def test_skill_failure_does_not_advance(self):
        ...
        _run(on_skill_check_passed(db, char, "search", False, difficulty=9))
...
class TestNpcs(_RealCorpusBase):
    ...
    def test_giver_present_in_start_room(self):
        ...
        # Display name of burning_deck_cantina.
        self.assertEqual(giver["room"],
    ...
    def test_npc_file_wired_into_era(self):
        ...
        self.assertIn(
            "npcs_drop_generalized_questline_ghost_shipment.yaml", npc_refs)
