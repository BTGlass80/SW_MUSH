"""
tests/test_mob_grind_tatooine_cantina_market.py

Static invariant tests for the Tatooine Cantina & Market hostile mob grind
batch (data/worlds/clone_wars/npcs_drop_mob_grind_tatooine_cantina_market.yaml).

Covers 6 previously-uncovered Tatooine rooms across the cantina and market zones:
...
Verifies:
  ...
  9. Era cleanness — no forbidden strings
  10. Required char_sheet fields present on every NPC
...
GRIND_FILE = os.path.join(
    DATA_DIR, "npcs_drop_mob_grind_tatooine_cantina_market.yaml"
)
...
def test_rooms_are_expected_target_rooms(grind_data):
    """Every NPC is placed in one of the six target cantina/market rooms."""
    for npc in grind_data["npcs"]:
        assert npc["room"] in EXPECTED_ROOMS, (
            f"{npc['name']}: room {npc['room']!r} is not a target cantina/market room"
        )
...
def test_era_cleanness(grind_data):
    ...
    for term in ERA_FORBIDDEN:
        assert term not in raw, (
            f"Era violation: {term!r} found in cantina/market mob grind data"
        )
...
def test_registered_in_era_yaml(era_data):
    ...
    assert "cantina_market" in raw, (
        "npcs_drop_mob_grind_tatooine_cantina_market.yaml not registered in era.yaml"
    )
