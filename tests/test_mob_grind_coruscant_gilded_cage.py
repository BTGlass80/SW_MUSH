"""
tests/test_mob_grind_coruscant_gilded_cage.py

Static invariant tests for the Coruscant Gilded Cage hostile mob grind batch
(data/worlds/clone_wars/npcs_drop_mob_grind_coruscant_gilded_cage.yaml).
...
Verifies:
  ...
  5. Weapon keys exist in data/weapons.yaml
  6. Era cleanness — no forbidden strings in any player-facing field
  7. Required char_sheet fields are present
  8. Schema version is 1
"""
...
GRIND_FILE = os.path.join(DATA_DIR, "npcs_drop_mob_grind_coruscant_gilded_cage.yaml")
...
EXPECTED_ROOMS = {
    "Coruscant - Gilded Cage - Alien Quarter",
    "Coruscant - Gilded Cage - Bazaar",
    "Coruscant - Gilded Cage - Courtyard",
}
...
def weapon_keys():
    with open(WEAPONS_FILE, encoding="utf-8") as f:
        content = f.read()
    # Extract weapon keys via simple string scan (file may have encoding issues
    # with round-trip yaml; use line-level parsing instead)
    keys = set()
...
def test_rooms_are_gilded_cage_rooms(grind_data):
    """Every NPC is placed in one of the three Gilded Cage public rooms."""
    for npc in grind_data["npcs"]:
        assert npc["room"] in EXPECTED_ROOMS, (
            f"{npc['name']}: room {npc['room']!r} is not a Gilded Cage room"
        )
...
def test_required_char_sheet_fields(grind_data):
    ...
    required = {"attributes", "skills", "weapon", "move", "force_points",
                "character_points", "dark_side_points"}
    required_attrs = {"dexterity", "knowledge", "mechanical", "perception",
                      "strength", "technical"}
    for npc in grind_data["npcs"]:
...
def test_era_cleanness(grind_data):
    ...
    for term in ERA_FORBIDDEN:
        assert term not in raw, (
            f"Era violation: {term!r} found in gilded_cage mob grind data"
        )
...
def test_registered_in_era_yaml(era_data):
    ...
    assert "gilded_cage" in raw, (
        "npcs_drop_mob_grind_coruscant_gilded_cage.yaml not registered in era.yaml"
    )
