# -*- coding: utf-8 -*-
"""
tests/test_generalized_questline_paper_death.py — T3.24 generalized
quest expansion, thirtieth slice.

Proves the THIRTIETH accessible (non-t5, non-tutorial) `kind: questline`
chain — "The Paper Death" (coruscant_paper_death) — is shipped correctly and
walks start->graduation through the PRODUCTION dispatcher, the same hooks the
live parser calls. Like the first twenty-nine slices it reuses the live
...
Firsts for the accessible-questline arc:
  * a THIRTIETH distinct skill spread — RUNNING (Dexterity) + MELEE COMBAT
    (Dexterity) — neither of which any prior accessible questline uses. It is
    the FIRST accessible questline to reward the MANHUNTER / SKIP-TRACER build,
    the FIRST to use RUNNING, and the FIRST to use MELEE COMBAT. (Honestly BOTH
    spread skills are Dexterity, so this is NOT a cross-attribute spread — it is
    the SECOND all-Dexterity-weapon-pool spread, after The Empty Proof's firearms
    + thrown weapons; and it is NOT the first weapon arc — The Empty Proof's
    marksman came first. What is genuinely first is the running and melee-combat
    skills and the run-it-down-then-take-it-close pursuit pairing, pointedly
    distinct from The Empty Proof's two-RANGED-weapon marksman: this is a
    PURSUIT-and-CLOSE build, foot-speed to catch and a blade to take.);
  * set on CORUSCANT, in the southern_underground (undercity) zone
    (crystal_jewel_alley / crystal_jewel_back_rooms / ice_crypts / lower_medical),
    every room of which is FRESH to the entire chain corpus. The Buried Block
    already used southern_underground rooms, so this is honestly the SECOND arc
    on the Coruscant undercity face (the EIGHTH Coruscant arc overall), NOT a
    fresh face; what is fresh is the four-room ROOM CLUSTER. Only the step-4
    combat room must be combat-capable (lower_medical is contested); the
    giver/return room and the two skill rooms host no fight. The
    Senate/Jedi/Republic war effort have no reach at a lower-city debtor written
    dead and never appear; the racket is run afoul of by the lower-city bond
    registry that voids the death, the way every prior accessible arc keeps the
  ...
  * a foil who carries the proven blaster_pistol of the ranged foils, squarely
    ...
    Salted Lane's ... The Rolled Log's foils), pointedly NOT the melee line of
    The Condemned Hull's out-of-band Houk foil.

The story shape is new too — breaking a FALSE-DEATH BOND FRAUD, the first
accessible arc whose racket is the fabrication of a living person's DEATH for
profit. A lower-city bond-broker files a debtor CONFIRMED DEAD when she is worth
more dead than found: a bought clinic orderly forges a death certificate, a
salted stand-in body is logged into the freezing Ice Crypts where the cold
defeats a forensic read, and the broker collects the death-clause payout and
seizes the estate while the living woman is written out of existence and, if she
surfaces, hunted to make the paper death real. So run the written-dead debtor
down through the letting-rooms before the crew reaches her (running), take the
body-handler close and quiet with a blade at the crypt to seize the salted
stand-in and the crypt-log (melee combat), stand off the enforcer at the clinic
(combat_won), and stand the living woman and the salted stand-in side by side
for the bond registry. Pointedly DISTINCT from the prior arcs it might be
mistaken for: The Bonded Crew BONDS people into labor (they live, worked); The
Buried Block declares a BUILDING derelict; The Wasting Ward / The Dust-Sick fake
an AILMENT on the healthy; The Rolled Log rolls back a CRAFT's history — here a
living PERSON's death is fabricated whole. It carries a real combat climax
...
QUESTLINE_ID = "coruscant_paper_death"
ACHIEVEMENT_KEY = "paper_death_cleared"
GIVER_NPC = "Nima Tarr"
ANTAGONIST_NPC = "Tovek Grahl"
ENEMY_TEMPLATE = "paper_death_enforcer"
START_ROOM = "crystal_jewel_alley"
GIVER_ROOM_NAME = "Coruscant - Crystal Jewel Alley"
ANTAGONIST_ROOM_NAME = "Coruscant - Lower City Medical Post"
COMBAT_ROOM_SLUG = "lower_medical"
STARTING_ZONE = "southern_underground"
NPC_FILE = (PROJECT_ROOT / "data" / "worlds" / "clone_wars"
            / "npcs_drop_generalized_questline_paper_death.yaml")

# The thirtieth skill spread, in step order (steps 2/3 are skill_check_passed).
EXPECTED_SKILLS = ["running", "melee combat"]

# The skill spreads of the prior TWENTY-NINE accessible questlines (each
# non-combat skill that gates a skill_check_passed step). The thirtieth
# spread must share NO skill with any of them — the "thirtieth DISTINCT
# spread" claim.
PRIOR_SPREAD_SKILLS = {
...
class TestWalkthrough(_RealCorpusBase):
    ...
    def test_full_walkthrough_to_graduation(self):
        ...
        # Step 1: talk to Nima Tarr (the skip-tracer)
        _run(on_talk_to_npc(db, char, GIVER_NPC))
        self.assertEqual(_qstate(char).get("step"), 2)

        # Step 2: run Oly Fenn down in the letting-rooms (running)
        _run(on_skill_check_passed(db, char, "running", True, difficulty=11))
        self.assertEqual(_qstate(char).get("step"), 3)

        # Step 3: take the body-handler at the crypt (melee combat)
        _run(on_skill_check_passed(db, char, "melee combat", True,
                                   difficulty=13))
        self.assertEqual(_qstate(char).get("step"), 4)

        # Step 4: stop Tovek Grahl at the clinic (combat_won, count 1)
        _run(on_combat_won(db, char, ENEMY_TEMPLATE, 1))
        self.assertEqual(_qstate(char).get("step"), 5)

        # Step 5: return to Nima Tarr -> graduate
        _run(on_talk_to_npc(db, char, GIVER_NPC))
    ...
    def test_skill_failure_does_not_advance(self):
        ...
        _run(on_skill_check_passed(db, char, "running", False, difficulty=11))
        self.assertEqual(_qstate(char).get("step"), 2)  # no advance

    def test_wrong_skill_does_not_advance(self):
        # Step 2 gates on running; a passing melee-combat check (this
...
class TestReachabilityBits(_RealCorpusBase):
    ...
    def test_skill_spread_is_running_then_melee(self):
        # The thirtieth distinct spread: the two skill_check_passed steps gate
        # on running then melee combat (no prior accessible questline uses
        # either).
        ql = self._questline()
    ...
    def test_spread_is_distinct_from_all_prior_arcs(self):
        # The "thirtieth DISTINCT spread" claim: neither spread skill is used
        # by any of the prior twenty-nine accessible questlines.
        self.assertFalse(
    ...
    def test_spread_is_all_dexterity_not_a_cross(self):
        # The defining build: the MANHUNTER / SKIP-TRACER — running (the legs
        # that run a fleeing mark down) + melee combat (the blade that takes her
        # close). Grounded against the live skills.yaml: BOTH are DEXTERITY
        # skills. This is the HONEST framing — NOT a cross-attribute spread, but
        # the SECOND all-Dexterity-weapon-pool spread (after The Empty Proof's
        # firearms + thrown weapons); the genuine firsts are the running and
        # melee-combat skills and the run-then-take pursuit pairing, asserted
        # elsewhere.
    ...
    def test_no_prior_arc_used_running_or_melee_combat(self):
        # The "first running / first melee-combat arc" claim: no prior
        # accessible arc used either skill. (The one prior Dexterity arc, The
        # Empty Proof, used firearms + thrown weapons — never running or melee
        # combat.)
        self.assertNotIn("running", PRIOR_SPREAD_SKILLS,
                         "a prior arc already used running — the 'first "
                         "running arc' claim is false")
        self.assertNotIn("melee combat", PRIOR_SPREAD_SKILLS,
                         "a prior arc already used melee combat — the 'first "
                         "melee-combat arc' claim is false")
    ...
    def test_spread_skills_resolve_to_trained_pools(self):
        ...
        trained = {
            ...
            "skills": json.dumps({"running": "5D"}),
        }
        untrained = {
            "attributes": json.dumps({"dexterity": "3D"}),
            "skills": json.dumps({}),
        }
        trained_pool = _get_skill_pool(trained, "running", reg)
        raw_pool = _get_skill_pool(untrained, "running", reg)
        # A char who trained Running must roll a STRICTLY larger pool than a
        ...
        self.assertGreater(
            ...
            "authored spread skill 'running' must roll the trained pool, not "
            "raw Dexterity")
    ...
    def test_all_step_rooms_in_southern_underground_zone(self):
        # The "Coruscant undercity" claim: every step room (and the drop room)
        # is a real loaded southern_underground-zone room.
        underground = _underground_slugs()
        ql = self._questline()
        for step in ql.steps:
            self.assertIn(step.location, underground,
                          f"step {step.step} location {step.location!r} is not "
                          f"a southern_underground room — the 'Coruscant "
                          f"undercity' claim is false")
        self.assertIn(ql.graduation.drop_room, underground)
    ...
    def test_combat_room_is_not_secured(self):
        ...
        # lower-city clinic is contested. (The giver/return and skill rooms
        # host no fight, so their security is immaterial.)
        ql = self._questline()
    ...
    def test_every_room_is_fresh_to_the_corpus(self):
        ...
        # by ANY other chain in the corpus (in particular none of The Buried
        # Block's southern_underground rooms).
        ql = self._questline()
        ...
        self.assertFalse(
            overlap,
            f"The Paper Death reuses rooms already in the chain corpus "
...
class TestNpcs(_RealCorpusBase):
    ...
    def test_giver_present_in_start_room(self):
        ...
        # Display name of crystal_jewel_alley.
    ...
    def test_antagonist_carries_proven_ranged_weapon(self):
        # Back in the proven beatable band: Tovek Grahl carries blaster_pistol
    ...
    def test_giver_spread_embodies_the_quest_skills(self):
        # The giver's sheet is skewed to the legs-and-blade of a manhunter — she
        # embodies the two skills the quest sends a hand to use.
        giver = self.by_name[GIVER_NPC]
    ...
    def test_exactly_two_placed_npcs(self):
        ...
        # bond-broker (Dral Vante), the bought clinic orderly, the lenders and
        # insurers, the salted stand-in body, the crypt-curators, the living
        # debtor (Oly Fenn), the real doctor, and the bond registry are
        # narrated-only.
        self.assertEqual(len(self.npcs), 2,
                         "The Paper Death should place exactly two NPCs "
    ...
    def test_npc_file_wired_into_era(self):
        ...
        self.assertIn(
            "npcs_drop_generalized_questline_paper_death.yaml", npc_refs)
