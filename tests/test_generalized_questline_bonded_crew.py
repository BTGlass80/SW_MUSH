# -*- coding: utf-8 -*-
"""
tests/test_generalized_questline_bonded_crew.py — T3.24 generalized
quest expansion, twenty-first slice.

Proves the TWENTY-FIRST accessible (non-t5, non-tutorial) `kind: questline`
chain — "The Bonded Crew" (kuat_city_bonded_crew) — is shipped correctly and walks
start->graduation through the PRODUCTION dispatcher, the same hooks the live parser
calls. Like the first twenty slices (The Ghost Shipment, The Crooked Wheel, The Lost
Courier, The Skimmed Line, The Dust-Sick, The False Provenance, The Forged Notice,
The Warrens Toll, The Sealed Ledger, The Sabotaged Run, The Hollow Crew, The Driven
Herd, The Condemned Hull, The Long Haul, The Twisted Word, The Wasting Ward, The
Rigged Issue, The Salted Lane, The Short Weight, The Wreckers' Light) it reuses the
...
Firsts for the accessible-questline arc:
  * a TWENTY-FIRST distinct skill spread — BUSINESS (Knowledge) + INTIMIDATION
    ...
    gunnery / powersuit-operation+lifting / communications+planetary-systems). The
    FIRST accessible questline to reward the LEDGER-AND-LEVERAGE / FORENSIC-PRESSURE
    build — an audit desk and a cornered clerk. It is mechanically and thematically
    DISTINCT from the only two prior arcs that touch money and records, The Skimmed
    Line (security + bargain, a freelance RECOVERY that bypasses a lock and
    negotiates a return — not an audit) and The Sealed Ledger (pick pocket + hide +
    computer programming, a HEIST that lifts and slices a sealed record — not a
    forensic read of an open one);
  * the THIRD questline set on KUAT and the FIRST on the planetside KUAT CITY surface
    (kuat_city_landing_pad / kuat_city_main_street / kuat_mercantile_forum /
    kuat_city_hotel — the Ten merchant houses' mercantile/aristocratic ground city),
    every room of which is FRESH to the entire chain corpus — reusing NONE of The
    Skimmed Line's orbital-ring rooms or The Condemned Hull's and The Salted Lane's
    main-spaceport rooms. The Republic procurement liaison the Kuat City room text
    names stays distant background — and the outfit is run afoul of in the end by the
    merchant houses' own contract arbiter — so the war stays offstage the way every
  ...
  * a foil who carries the proven blaster_pistol of the ranged foils, squarely in
    ...
    Lane's, The Short Weight's, and The Wreckers' Light's foils).

The story shape is new too — breaking a DEBT-BONDAGE / PADDED-ADVANCE (labor)
racket; no prior accessible arc touches one. The Skimmed Line RECOVERS wages a
foreman skimmed off the top of a crew's pay; this does not skim the top — it inflates
the BOTTOM, engineering each worker's recruitment debt so it compounds faster than the
work can ever pay it down, and the crew is held by the books, not robbed by them. A
labor-contracting outfit recruits offworld crews on a "passage advance," books the
fare/lodging/tools at inflated rates against company scrip with compounding carry,
pays the logged hours back in scrip at a discount, and so engineers a debt that can
never clear — so reconcile the advance ledger against the real costs and the crew's
logged hours to prove the debt engineered (business), corner the outfit's junior
contracts-clerk and lean on him until he gives up the true second ledger (intimidation),
stand off the outfit's collector who comes for the proof (combat_won), and get the
audit and the ledger to the merchant houses' contract arbiter. It carries a real
...
achievement, real Kuat room slugs, the giver + foil NPCs, the combat-climax structure,
the all-Kuat-City-rooms + every-room-fresh-to-corpus claim, the twenty-first-distinct-
spread + business(Knowledge)/intimidation(Knowledge) claim + distinctness from the two
prior money/records arcs, and that the authored spread skills resolve to a trained
...
QUESTLINE_ID = "kuat_city_bonded_crew"
ACHIEVEMENT_KEY = "bonded_crew_cleared"
GIVER_NPC = "Nabo Threll"
ANTAGONIST_NPC = "Garvo Naak"
ENEMY_TEMPLATE = "bonded_crew_enforcer"
START_ROOM = "kuat_city_landing_pad"
GIVER_ROOM_NAME = "Kuat City - Landing Pad"
ANTAGONIST_ROOM_NAME = "Kuat City - The Andris Grand Hotel"
NPC_FILE = (PROJECT_ROOT / "data" / "worlds" / "clone_wars"
            / "npcs_drop_generalized_questline_bonded_crew.yaml")

# The twenty-first skill spread, in step order (steps 2/3 are skill_check_passed).
EXPECTED_SKILLS = ["business", "intimidation"]

# The skill spreads of the prior TWENTY accessible questlines (each non-combat
# skill that gates a skill_check_passed step). The twenty-first spread must share
# NO skill with any of them — the "twenty-first DISTINCT spread" claim.
PRIOR_SPREAD_SKILLS = {
...
# The only two prior arcs to touch money and records — a freelance RECOVERY that
# bypasses a lock and negotiates a return (The Skimmed Line) and a HEIST that lifts
# and slices a sealed record (The Sealed Ledger). The twenty-first's forensic-audit
# spread shares no spread skill with either.
SKIMMED_LINE_SPREAD = {"security", "bargain"}
SEALED_LEDGER_SPREAD = {"pick pocket", "hide", "computer programming"}
...
class TestWalkthrough(_RealCorpusBase):
    ...
    def test_full_walkthrough_to_graduation(self):
        ...
        # Step 1: talk to Nabo Threll (the foreman)
        _run(on_talk_to_npc(db, char, GIVER_NPC))
        self.assertEqual(_qstate(char).get("step"), 2)

        # Step 2: reconcile the advance ledger (business)
        _run(on_skill_check_passed(db, char, "business", True,
                                   difficulty=11))
        self.assertEqual(_qstate(char).get("step"), 3)

        # Step 3: corner the clerk for the true ledger (intimidation)
        _run(on_skill_check_passed(db, char, "intimidation", True,
                                   difficulty=13))
        self.assertEqual(_qstate(char).get("step"), 4)

        # Step 4: stop Garvo Naak in the Andris Grand (combat_won, count 1)
        _run(on_combat_won(db, char, ENEMY_TEMPLATE, 1))
        self.assertEqual(_qstate(char).get("step"), 5)

        # Step 5: return to Nabo Threll -> graduate
        _run(on_talk_to_npc(db, char, GIVER_NPC))
    ...
    def test_skill_failure_does_not_advance(self):
        ...
        _run(on_skill_check_passed(db, char, "business", False,
                                   difficulty=11))
        self.assertEqual(_qstate(char).get("step"), 2)  # no advance

    def test_wrong_skill_does_not_advance(self):
        # Step 2 gates on business; a passing intimidation check (this
...
class TestReachabilityBits(_RealCorpusBase):
    ...
    def test_skill_spread_is_business_then_intimidation(self):
        # The twenty-first distinct spread: the two skill_check_passed steps gate
        # on business then intimidation (no prior accessible questline uses
        # either).
        ql = self._questline()
    ...
    def test_spread_is_distinct_from_all_prior_arcs(self):
        # The "twenty-first DISTINCT spread" claim: neither spread skill is used
        # by any of the prior twenty accessible questlines.
        self.assertFalse(
    ...
    def test_spread_is_ledger_and_leverage(self):
        # The defining first: the spread is the LEDGER-AND-LEVERAGE / FORENSIC-
        # PRESSURE build — business (Knowledge, the audit desk) + intimidation
        # (Knowledge, the cornered clerk). Grounded against the live skills.yaml.
        ...
        self.assertIn("business", knowledge,
                      "'business' is not a Knowledge skill — the audit-desk "
                      "claim is false")
        self.assertIn("intimidation", knowledge,
                      "'intimidation' is not a Knowledge skill — the "
                      "cornered-clerk claim is false")

    def test_distinct_from_prior_money_and_records_arcs(self):
        # The differentiation from the only two prior arcs that touch money and
        # records: The Skimmed Line (security + bargain, a freelance RECOVERY)
        # and The Sealed Ledger (pick pocket + hide + computer programming, a
        # HEIST). This is a forensic AUDIT and shares NO spread skill with
        # either.
        self.assertFalse(
            set(EXPECTED_SKILLS) & SKIMMED_LINE_SPREAD,
            "The Bonded Crew shares a spread skill with The Skimmed Line — "
            "the 'audit, not a recovery' claim is false")
        self.assertFalse(
            set(EXPECTED_SKILLS) & SEALED_LEDGER_SPREAD,
            "The Bonded Crew shares a spread skill with The Sealed Ledger — "
            "the 'forensic read of an open ledger, not a heist of a sealed "
            "one' claim is false")
    ...
    def test_spread_skills_resolve_to_trained_pools(self):
        ...
        trained = {
            ...
            "skills": json.dumps({"business": "5D"}),
        }
        untrained = {
            "attributes": json.dumps({"knowledge": "3D"}),
            "skills": json.dumps({}),
        }
        trained_pool = _get_skill_pool(trained, "business", reg)
        raw_pool = _get_skill_pool(untrained, "business", reg)
        # A char who trained Business must roll a STRICTLY larger pool than a
        # char rolling raw Knowledge — proving the authored skill resolves to
        # the trained skill rather than the bare attribute.
        self.assertGreater(
            trained_pool, raw_pool,
            "authored spread skill 'business' must roll the trained "
            "pool, not raw Knowledge")

    def test_all_step_rooms_are_real_kuat_rooms(self):
        # The "third questline set on Kuat / Kuat City surface face" claim:
        ...
        for step in ql.steps:
            self.assertIn(step.location, kuat,
                          ...
                          f"a Kuat room — the 'third Kuat arc' claim is false")
    ...
    def test_every_room_is_fresh_to_the_corpus(self):
        ...
        # ANY other chain in the corpus (in particular none of The Skimmed
        # Line's orbital-ring rooms or The Condemned Hull's / The Salted Lane's
        # main-spaceport rooms).
        ql = self._questline()
        ...
        self.assertFalse(
            overlap,
            f"The Bonded Crew reuses rooms already in the chain corpus "
...
class TestNpcs(_RealCorpusBase):
    ...
    def test_giver_present_in_start_room(self):
        ...
        # Display name of kuat_city_landing_pad.
    ...
    def test_antagonist_carries_proven_ranged_weapon(self):
        # Back in the proven beatable band: Garvo Naak carries blaster_pistol
    ...
    def test_exactly_two_placed_npcs(self):
        ...
        # labor contractor (Sennick Drale), his offworld scrip-fence partner,
        # the padded advance ledger and the true second ledger, the frightened
        # junior contracts-clerk who keeps it, the bonded offworld crews, the
        # Ten Kuati merchant houses, the Republic procurement liaison, and the
        # merchant houses' contract arbiter are narrated-only.
        self.assertEqual(len(self.npcs), 2,
                         "The Bonded Crew should place exactly two NPCs "
    ...
    def test_npc_file_wired_into_era(self):
        ...
        self.assertIn(
            "npcs_drop_generalized_questline_bonded_crew.yaml", npc_refs)
