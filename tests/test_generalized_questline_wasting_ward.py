# -*- coding: utf-8 -*-
"""
tests/test_generalized_questline_wasting_ward.py — T3.24 generalized
quest expansion, sixteenth slice.

Proves the SIXTEENTH accessible (non-t5, non-tutorial) `kind: questline`
chain — "The Wasting Ward" (nar_shaddaa_wasting_ward) — is shipped correctly
and walks start->graduation through the PRODUCTION dispatcher, the same hooks
the live parser calls. Like the first fifteen slices (The Ghost Shipment, The
...
Haul, The Twisted Word) it reuses the live questline engine (active_questline
slot, the existing event types, the four reward funnels) with NO new engine
code, per quest_expansion_postlaunch_path_v1.md.

Firsts for the accessible-questline arc:
  * a SIXTEENTH distinct skill spread — MEDICINE + SCHOLAR — neither of which
    ...
    repair+astrogation / ground-vehicle-repair+ground-vehicle-operation /
    languages+cultures). `medicine` is a TECHNICAL skill and `scholar` a
    KNOWLEDGE skill — the FIRST accessible questline to reward the DOCTOR /
    DIAGNOSTICIAN-RESEARCHER (physician) build (read a board-"treated" patient
    and prove the wasting a poisoning, then run the agent down to its source).
    It is mechanically DISTINCT from The Dust-Sick, the only prior medical arc,
    which rewards FIRST AID + SURVIVAL — a frontier FIELD MEDIC stabilizing a
    genuinely stricken patient in the wilderness: this rewards MEDICINE (clinical
    diagnosis, not battlefield stabilization) + SCHOLAR (research, not desert
    survival), a physician UNMASKING a manufactured illness in the city;
  * the FOURTH questline set on NAR SHADDAA and the first on the lower
    REFUGEE-AND-UNDERCITY face (refugee_sector / black_market_medcenter /
    droid_junkyard / undercity_market), every room of which is FRESH to the entire
    chain corpus — reusing NONE of The Ghost Shipment's cantina/Undercity-Depths,
    The Forged Notice's upper-dock registry, or The Warrens Toll's Warrens rooms.
    The Clone War the refugee_sector room description names (the newest arrivals
    "from CIS-occupied worlds") stays distant background — the displaced are the
    racket's prey, not its cause — so the war stays offstage the way every prior
    accessible arc keeps the Republic/CIS conflict offstage;
  * a foil who carries the proven blaster_pistol of the ranged foils, squarely
    in the proven beatable band.

The story shape is new too — breaking a FAKE-PLAGUE / QUARANTINE-EXTORTION
(medical-fraud) racket (no prior accessible arc touches one; The Dust-Sick is a
medical RESCUE of a real patient, not the UNMASKING of a fake epidemic): a
Hutt-lieutenant's self-styled "sanitation board" declares a contagious "wasting
fever" loose in the refugee sector, seals the level under quarantine, sets a
clearance toll, and sells the only "cure," so its backers can buy the emptied,
condemned habs cheap as the families who can't pay sicken or flee, so read a
board-"treated" patient and prove the wasting a poisoning, not a contagion, by
how it travels (medicine), trace the cut to the droid junkyard it's scavenged up
out of and name the agent in the board's own cure (scholar), stop the board's collector
who comes for the proof at the undercity market (combat_won), and get the truth
to the Moon's port-medical registry. It carries a real combat climax (step 4),
...
NPCs, the combat-climax structure, the all-Nar-Shaddaa-rooms + every-room-fresh-
to-corpus claim, the sixteenth-distinct-spread + Technical+Knowledge physician
...
QUESTLINE_ID = "nar_shaddaa_wasting_ward"
ACHIEVEMENT_KEY = "wasting_ward_cleared"
GIVER_NPC = "Ulil Maan"
ANTAGONIST_NPC = "Greel Vodo"
ENEMY_TEMPLATE = "wasting_ward_enforcer"
START_ROOM = "refugee_sector"
GIVER_ROOM_NAME = "Nar Shaddaa - Refugee Sector"
ANTAGONIST_ROOM_NAME = "Nar Shaddaa - Undercity Market"
NPC_FILE = (PROJECT_ROOT / "data" / "worlds" / "clone_wars"
            / "npcs_drop_generalized_questline_wasting_ward.yaml")

# The sixteenth skill spread, in step order (steps 2/3 are skill_check_passed).
EXPECTED_SKILLS = ["medicine", "scholar"]

# The skill spreads of the prior FIFTEEN accessible questlines (each non-combat
# skill that gates a skill_check_passed step). The sixteenth spread must share
# NO skill with any of them — the "sixteenth DISTINCT spread" claim.
PRIOR_SPREAD_SKILLS = {
...
# The rooms the three prior Nar Shaddaa arcs use — the sixteenth arc must reuse
# NONE of them (part of the "fresh face" claim).
NAR_SHADDAA_PRIOR_ROOMS = {
    # The Ghost Shipment
    "burning_deck_cantina", "undercity_depths",
    # The Forged Notice (upper docks / freight registry)
    "upper_dock_observation", "westport_arrivals",
    "smugglers_guild_hall", "enforcer_alley",
    # The Warrens Toll
    "warrens_squat_block", "warrens_scavenger_den",
    "warrens_reactor_core", "warrens_collapsed_plaza",
}
...
class TestWalkthrough(_RealCorpusBase):
    ...
    def test_full_walkthrough_to_graduation(self):
        ...
        # Step 1: talk to Ulil Maan (the medic)
        _run(on_talk_to_npc(db, char, GIVER_NPC))
        self.assertEqual(_qstate(char).get("step"), 2)

        # Step 2: prove the wasting is a poisoning (medicine)
        _run(on_skill_check_passed(db, char, "medicine", True,
                                   difficulty=11))
        self.assertEqual(_qstate(char).get("step"), 3)

        # Step 3: name the agent in the cure (scholar)
        _run(on_skill_check_passed(db, char, "scholar", True,
                                   difficulty=13))
        self.assertEqual(_qstate(char).get("step"), 4)

        # Step 4: stop Greel Vodo at the undercity market (combat_won, count 1)
        _run(on_combat_won(db, char, ENEMY_TEMPLATE, 1))
        self.assertEqual(_qstate(char).get("step"), 5)

        # Step 5: return to Ulil Maan -> graduate
        _run(on_talk_to_npc(db, char, GIVER_NPC))
    ...
    def test_skill_failure_does_not_advance(self):
        ...
        _run(on_skill_check_passed(db, char, "medicine", False,
                                   difficulty=11))
        self.assertEqual(_qstate(char).get("step"), 2)  # no advance

    def test_wrong_skill_does_not_advance(self):
        # Step 2 gates on medicine; a passing scholar check (this questline's
...
class TestReachabilityBits(_RealCorpusBase):
    ...
    def test_skill_spread_is_medicine_then_scholar(self):
        # The sixteenth distinct spread: the two skill_check_passed steps gate
        # on medicine then scholar (no prior accessible questline uses either).
        ql = self._questline()
    ...
    def test_spread_is_distinct_from_all_prior_arcs(self):
        # The "sixteenth DISTINCT spread" claim: neither spread skill is used by
        # any of the prior fifteen accessible questlines.
        self.assertFalse(
    ...
    def test_spread_is_technical_knowledge_physician(self):
        # The defining first: the spread is the DOCTOR / DIAGNOSTICIAN-RESEARCHER
        # build — medicine (Technical) + scholar (Knowledge). Grounded against
        # the live skills.yaml.
        ...
        self.assertIn("medicine", technical,
                      "'medicine' is not a Technical skill — the physician "
                      "claim is false")
        self.assertIn("scholar", knowledge,
                      "'scholar' is not a Knowledge skill — the researcher "
                      "claim is false")

    def test_distinct_from_dust_sick_field_medic(self):
        # The differentiation from the only prior medical arc: The Dust-Sick
        # rewards first_aid + survival (field medic); this rewards medicine +
        # scholar (physician). They share NO spread skill.
        dust_sick = {"first aid", "survival"}
        self.assertFalse(
            set(EXPECTED_SKILLS) & dust_sick,
            "The Wasting Ward shares a spread skill with The Dust-Sick — the "
            "'distinct from the field medic' claim is false")
    ...
    def test_spread_skills_resolve_to_trained_pools(self):
        ...
        trained = {
            ...
            "skills": json.dumps({"medicine": "5D"}),
        }
        untrained = {
            "attributes": json.dumps({"technical": "3D"}),
            "skills": json.dumps({}),
        }
        trained_pool = _get_skill_pool(trained, "medicine", reg)
        raw_pool = _get_skill_pool(untrained, "medicine", reg)
        # A char who trained Medicine must roll a STRICTLY larger pool than a
        # char rolling raw Technical — proving the authored skill resolves to the
        # trained skill rather than the bare attribute.
        self.assertGreater(
            trained_pool, raw_pool,
            "authored spread skill 'medicine' must roll the trained pool, not "
            "raw Technical")

    def test_all_step_rooms_are_nar_shaddaa_rooms(self):
        # The "fourth questline set on Nar Shaddaa" claim: every step room (and
        # the drop room) is a real loaded Nar Shaddaa room.
        nar = _nar_shaddaa_room_slugs()
        ql = self._questline()
        for step in ql.steps:
            self.assertIn(step.location, nar,
                          f"step {step.step} location {step.location!r} is not "
                          f"a Nar Shaddaa room — the 'fourth Nar Shaddaa arc' "
                          f"claim is false")
    ...
    def test_every_room_is_fresh_to_the_corpus(self):
        ...
        self.assertFalse(
            overlap,
            f"The Wasting Ward reuses rooms already in the chain corpus "
            f"{overlap} — the 'every room fresh' claim is false")
        self.assertFalse(
            used & NAR_SHADDAA_PRIOR_ROOMS,
            "The Wasting Ward reuses a prior Nar Shaddaa arc's room — the "
...
class TestNpcs(_RealCorpusBase):
    ...
    def test_giver_present_in_start_room(self):
        ...
        # Display name of refugee_sector.
    ...
    def test_antagonist_carries_proven_ranged_weapon(self):
        # Back in the proven beatable band: Greel Vodo carries blaster_pistol
    ...
    def test_exactly_two_placed_npcs(self):
        ...
        # Hutt lieutenant, the sanitation board, its inspectors, the "cure" and
        # clearance toll, the off-books buyers, the cut-merchant, the treated
        # patient, the dying refugees, and the medcenter's ambient doctor are
        # narrated-only.
        self.assertEqual(len(self.npcs), 2,
                         "The Wasting Ward should place exactly two NPCs "
    ...
    def test_npc_file_wired_into_era(self):
        ...
        self.assertIn(
            "npcs_drop_generalized_questline_wasting_ward.yaml", npc_refs)
