# -*- coding: utf-8 -*-
"""
tests/test_drop_mob_grind_coruscant_midlevels.py

Verifies the Coruscant Mid-Levels & Commercial District hostile-mob grind
batch (data/worlds/clone_wars/npcs_drop_mob_grind_coruscant_midlevels.yaml).

Six NPCs seeded across The Outlander Club, Gartro Rookery, Central Power
Grid Station, University of Coruscant District, Coco Town Loft District,
and Coco Town Civic Block.
...
MOB_FILE = os.path.join(CW_DIR, "npcs_drop_mob_grind_coruscant_midlevels.yaml")
...
class TestEraManifestRef(unittest.TestCase):
    def test_file_in_era_yaml(self):
        ...
        ref_name = "npcs_drop_mob_grind_coruscant_midlevels.yaml"
        self.assertIn(
            ref_name, npcs_refs,
