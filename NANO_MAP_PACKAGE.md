...
> **Drop 4.18:** this wrapper is now the canonical `MASTER_PROMPT` in
> `tools/paint_brief_common.py`. The wilderness generator emits a fully-filled
> copy per region (`<slug>_paint_brief.md`) — for those regions you don't fill
> the slots by hand, you paste the generated prompt. The text below is kept for
> reference and for the city briefs (§3) until the city generator lands.
...
## 6. WILDERNESS ADDENDUM (Drop 4.16) — Dune Sea + Coruscant Underworld

Wilderness *region overview* maps (the navigator's Tier-1b) paint the same way as
the cities, with one difference in how the seed reads: it carries **soft terrain
zones** and **faint tracks** instead of hard districts and bright causeways. Same
**style clause** (§2.5) and the **same shared style-reference image** as the cities
— the atlas stays unified.

### 6.1 Seeds to feed
Generated by `tools/make_substrate_seed.py --wilderness --tight` from the region
overview specs (`data/worlds/clone_wars/maps/<area_key>_overview.yaml`):

- `static/tools/seeds/tatooine_dune_sea_tight_seed.png`
- `static/tools/seeds/coruscant_underworld_tight_seed.png`

Matching `*_tight_keymap.png` are your labeled references — **never feed the keymap**
(it has text). Loose (more-blurred) `*_seed.png` previews also exist; feed the
**tight** ones to Nano.

### 6.2 Prompt — reuse §2.5 verbatim, with TWO bullet substitutions
Keep the §2.5 master prompt and its **Style** paragraph exactly. For wilderness seeds
only, read these two layout bullets as:

- *each solid colored region is a* **terrain zone** *— repaint as the natural ground
  described (dunes, rock flats, canyon, ferrocrete ruin, dark depths), blending
  softly into its neighbors; NOT a hard-edged district.*
- *the faint pale lines are* **tracks / trails** *— keep them as faint worn paths
  across the terrain; do NOT turn them into bright paved roads.*

The **gold-block** bullet is unchanged (each gold block = a distinct landmark of the
type described, painted on that exact spot). Wilderness seeds have **no** faint
density rectangles, so ignore that bullet.

### 6.3 Per-region briefs — now GENERATED (Drop 4.18)

> **You no longer hand-fill `{GEOGRAPHY}` for wilderness.** The generator
> (`tools/gen_wilderness_overview.py`) emits a complete, ready-to-paste Nano
> prompt per region at **`static/tools/seeds/<slug>_paint_brief.md`**. Its
> placement (relative regions), per-feature visuals (pulled from each
> landmark's authored `short_desc`), and terrain texture are all derived from
> the navigable grid — the same source as the seed — so the prompt cannot
> drift from the map. Open the brief, copy the fenced prompt, attach the named
> tight seed, paint.

Per region:
- **Dune Sea (Tatooine):** brief `static/tools/seeds/tatooine_dune_sea_paint_brief.md`
  — feed `tatooine_dune_sea_tight_seed.png` — save `static/maps/tatooine_dune_sea_substrate.png`.
- **Coruscant Underworld:** brief `static/tools/seeds/coruscant_underworld_paint_brief.md`
  — feed `coruscant_underworld_tight_seed.png` — save `static/maps/coruscant_underworld_substrate.png`.

The seed's gold blocks sit where the real landmarks are; **paint each named
feature on its block, don't relocate it.** The regions are sparse on purpose —
the brief tells the painter how many features there are and to keep open ground
between them. (Filenames are load-bearing — the manifest + generator expect
them. The city briefs in §3 are still hand-written until the city generator
lands.)

### 6.4 Re-import (per region — generator-driven, no manual pin-drag)

The pins are projected from the grid, so they are **already correct** — you do
NOT drag them to match the painting (that would re-introduce the drift this
pipeline removed). Registration is verification-only.

1. Save the painted PNG to `static/maps/<area_key>_substrate.png` (exact name).
2. **Re-run the generator:**
   `python3 tools/gen_wilderness_overview.py data/worlds/clone_wars/wilderness/dune_sea.yaml data/worlds/clone_wars/wilderness/coruscant_underworld.yaml`
   It detects the PNG on disk and auto-engages `substrate_image` in both the
   overview YAML and the live `m3_wilderness_overview_data.js`. Substrate-first
   then kicks in: the painting becomes the Tier-1b backdrop, the procedural base
   drops out, and the faithful POI pins/labels stay on top. Until the PNG exists,
   the region renders the procedural fallback (sparse faithful POIs on a tonal
   base) — nothing breaks.
3. *(Optional check)* open
   `http://<host>/static/tools/map_register.html?manifest=/static/tools/manifests/<area_key>.json`
   to eyeball that the painted features land near their pins. If a painted
   feature is well off its pin, it's easier to nudge the PROMPT and repaint than
   to move the pin — the pin is grid-truth.

### 6.5 Wilderness package contents (Drop 4.17 + 4.18)
- `tools/gen_wilderness_overview.py` — generates faithful overview specs, live JS region data, **and the per-region Nano paint brief** from the navigable grid (single source).
- `tools/paint_brief_common.py` — **NEW (4.18)**: shared paint-brief core (canonical master prompt, relative-placement language, authored-flavor extraction, density phrasing). The city generator will reuse it.
- `static/tools/seeds/{tatooine_dune_sea,coruscant_underworld}_paint_brief.md` — **GENERATED** ready-to-paste Nano prompts (placement + authored visuals + terrain, all grid-derived).
- `tools/gen_wilderness_overview.py` — **NEW**: generates faithful overview specs + live JS region data from the navigable grid (single source).
- `tools/make_substrate_seed.py` — `--wilderness` mode; projector is now no-flip (seed agrees with renderer + overview).
- `tools/make_register_manifest.py` — distinctive-icon set widened; fy no longer inverted (matches the y-down substrate).
- `data/worlds/clone_wars/maps/{tatooine_dune_sea,coruscant_underworld}_overview.yaml` — **GENERATED** faithful specs (do not hand-edit).
- `static/spa/m3_wilderness_overview_data.js` — **GENERATED** live Tier-1b region data; `resolveRegion` prefers it.
- `static/tools/seeds/{tatooine_dune_sea,coruscant_underworld}_tight_seed.png` — **feed these to Gemini** (faithful POI positions).
- `static/tools/seeds/..._tight_keymap.png` + loose previews — references.
- `static/tools/manifests/{tatooine_dune_sea,coruscant_underworld}.json` — verification pin manifests.
